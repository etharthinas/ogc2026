...
# =============================================================================
# See heuristics/heuristic_4.md. v3 placement/feasibility core kept verbatim; the
...
def _improve(prob_info, assignments, bays, bay_u, w1, w2, w3, deadline, forced):
    ...
    while time.time() < deadline:
        ...
        if kick:
            ...
            # union with a random chunk for a bigger jump
            extra = rng.sample(list(cur), min(len(cur), 8))
            removed = set(removed) | set(extra)
        elif rounds % 3 == 0:
            removed = _destroy_window(cur, blocks_data, rng)
        else:
            k = min(max(1, n // 20), 4 + (rounds % 8) * 3)
            removed = _destroy_tardy(cur, blocks_data, k, rng)
            if removed is None:
                # no tardiness left: try window destroy to chip obj2/obj3, else stop
                removed = _destroy_window(cur, blocks_data, rng)
        if not removed:
            break
...
def algorithm(prob_info, timelimit=60):
    ...
    if forced:
        # Forced/congested instances: ONE EDD-earliest construction (the secondary
        # congestion construction rarely won the best-of and wasted budget), then
        # hand the bulk of the time to the basin-hopping improver -- that is where
        # the gains are (prob_38 210M@90s -> 95M@300s came from the improver). Cap
        # construction at ~55% of the window so the improver always runs; at long
        # limits the construction finishes naturally well under the cap.
        cons_deadline = t_start + search_window * 0.55
        try:
            assign = _construct(prob_info, edd, bays, bay_u, w1, w2, w3,
                                t_start, cons_deadline, forced=True)
