...
_WIDE_RETRY = False
# jv19 RELEASE LOOKAHEAD (Route A1, heuristic_jv19.md): the drain look-set only
# ever contained already-queued blocks, so a placement at t could foreclose the
# burst releasing at t+1..t+delta (median release gap is 1 tick on 38/27, burst
# size 5-11, residency ~21 ticks). _RLOOK = delta ticks of lookahead (0 = off =
# byte-equal jv17 drain); _RLOOK_K caps the extra look entries so _drain_score
...
def _dispatch_construct(prob_info, bays, bay_u, w1, w2, w3, deadline, raster,
    ...
    def _imminent(t):
        """jv19 release lookahead: blocks releasing in (t, t+_RLOOK] join the
        drain look-set (most-urgent first, capped at _RLOOK_K) so a placement
        at t cannot foreclose the burst arriving one or two ticks later.
        rel_sorted[rp:] holds exactly the not-yet-released blocks at t."""
        lim = t + _RLOOK
        cand = []
        j = rp
        while j < n and rels[rel_sorted[j]] <= lim:
            b = rel_sorted[j]
            if targets is None or targets[b] <= lim:
                cand.append(b)
            j += 1
        if len(cand) > _RLOOK_K:
            cand.sort(key=lambda b: -atc(b, t))
            del cand[_RLOOK_K:]
        return cand
...
def _run_strategy(wid, prob_info, timelimit, t_start, push, inbox=None):
    ...
    def dispatch(kappa, gamma, drng=None, alpha=0.0, score_pos=True,
                 ...
                 drain=0, zone=0):
        # jv20 = jv19 + the SECOND dead kwarg found by the 07-24 AST sweep:
        # `zone` (jv6b r5 TEMPORAL ZONING seed for the 31-class). Same defect
        # as `drain` -- the call raised TypeError inside `except Exception:
        # pass`, so the zoning seed never ran either. Isolated in its own arm
        # so the jv19 A/B stays a one-variable question.
