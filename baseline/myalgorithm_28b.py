# myalgorithm_28b.py  --  v28b = v28a + ONE new improver destroy mode:
#   RESIDENT<->QUEUED 2-EXCHANGE (heuristic_28 Improvement B). Verdict
#   (ledger_27_diag): during giant bursts >=99.7% of queued-block moments have
#   NO exact-feasible earlier placement -- the saturated bays (~0.7 layer-0
#   density) are the binding constraint and 100% of tardiness is entry-queue
#   delay; the lever is WHICH blocks hold the saturated bays. `_destroy_tardy`
#   removes a tardy block alone -> re-inserting into the same saturated schedule
#   cannot move it earlier (no space). NEW `_destroy_blocker_swap`: for a
#   tardiness-weighted sampled tardy block i, use the near-miss raster scan to
#   find anchors at earlier entry times t in [release_i, entry_i) where i ALMOST
#   fits, name the small RESIDENT blocker set exactly (_blockers_at, mirrors
#   _can_place), require |blockers| <= BLOCKER_CAP (default 3) and every blocker
#   to have due-slack, then destroy {i} U blockers and repair with i FIRST (grabs
#   the earlier slot), blockers after. Wired as one more destroy mode in the
#   _improve() rotation, gated on best_tardy>0 (byte-neutral on zero-tardy
#   instances -- the new branch is never entered, so existing modes' rng streams
#   are untouched) and on a rounds cadence (SWAP_EVERY, no rng); rng is consumed
#   ONLY inside the fired branch. Obj-gated accept as usual (best_assign monotone,
#   swap is neither kick nor restart -> accepted only if strictly better).
#   BLOCKER_CAP is a module-visible constant (sweep {2,3} is a one-liner).
# ---------------------------------------------------------------------------
...
def _blockers_at(bay, rel, nb, entry, exit_t):
    """v28b: resident block_ids in `rel` (time-overlapping sched triples
    (block, a, e)) that OBSTRUCT placing `nb` at [entry, exit_t) -- the exact same
    conditions `_can_place` early-returns False on (collision, crane entry/exit
    blocking, and reverse entry/exit blocking), but COLLECTED into a set instead
    of short-circuiting. Names the resident blocker set for the resident<->queued
    2-exchange. Uses the same cached pairwise primitives as `_can_place`, so the
    verdict is exact (a block is a blocker iff its presence is why _can_place would
    reject nb here)."""
    nbid = nb.block_id
    blockers = set()
    for b, a, e in rel:
        hit = False
        if _overlaps(entry, exit_t, a, e) and _collide(bay, nb, b):
            hit = True
        if not hit and ((a < entry < e) or (a == entry and b.block_id < nbid)):
            if _entry_blocked(bay, b, nb):
                hit = True
        if not hit and ((a < exit_t < e) or (e == exit_t and b.block_id > nbid)):
            if _exit_blocked(bay, b, nb):
                hit = True
        if not hit and ((entry < a < exit_t) or (entry == a and nbid < b.block_id)):
            if _entry_blocked(bay, nb, b):
                hit = True
        if not hit and ((entry < e < exit_t) or (exit_t == e and nbid > b.block_id)):
            if _exit_blocked(bay, nb, b):
                hit = True
        if hit:
            blockers.add(b.block_id)
    return blockers
...
# -----------------------------------------------------------------------------
# v28b: RESIDENT<->QUEUED 2-EXCHANGE (heuristic_28 Improvement B).
# Constants near the other improver knobs so a sweep is a one-line change.
# -----------------------------------------------------------------------------
BLOCKER_CAP = 3          # v28b: max resident blockers ejected per swap (sweep {2,3})
SWAP_EVERY = 4           # v28b: rounds cadence at which the swap mode may fire
SWAP_TIME_SAMPLES = 6    # earlier entry times sampled from [release_i, entry_i)
SWAP_POS_CAP = 12        # near-miss anchors probed per (t, orient)


def _destroy_blocker_swap(cur, blocks_data, bays, raster, rng):
    """v28b (heuristic_28 Improvement B): the resident<->queued 2-exchange.

    ledger_27_diag verdict: during giant-instance bursts >=99.7% of queued-block
    moments have NO exact-feasible earlier placement -- the saturated bays are the
    binding constraint and 100% of tardiness is entry-queue delay. `_destroy_tardy`
    removes a tardy block alone, but re-inserting it into the SAME saturated
    schedule cannot move it earlier (no space). This move instead EJECTS the small
    set of RESIDENT blocks that block an earlier placement of a sampled tardy block
    i (found via the near-miss raster scan + exact `_blockers_at`), so i can grab
    the earlier slot; the residents -- which have due-slack -- reinsert afterwards.

    Returns (removed_set, priority_list) with i FIRST then blockers, or None (no
    valid swap this round -> caller falls through). rng is consumed ONLY inside
    this function, and only when tardy blocks exist, so the mode never displaces
    other modes' rng streams when it does not fire."""
    ...
    if not tardy:
        return None
    # tardiness-weighted sample of the block to pull earlier
    tot = sum(t for _, t in tardy)
    if tot <= 0:
        return None
    thresh = rng.random() * tot
    acc = 0.0
    i = tardy[-1][0]
    for bi, t in tardy:
        acc += t
    ...
    entry_i = int(ai["entry_time"])
    span = entry_i - rel
    if span <= 0:
        return None                     # already at release -> no earlier slot
    orients = [oi for oi in _unique_orients(blk) if _orient_fits(blk, oi, bay)]
    if not orients:
        return None
    # this bay's schedule triples (exclude i itself)
    sched_bay = [(_mkblock(bj, blocks_data[bj], a["x"], a["y"], a["orient_idx"]),
                  a["entry_time"], a["exit_time"])
                 for bj, a in cur.items()
                 if a["bay_id"] == bay_id and bj != i]
    # sample a few earlier entry times in [release_i, entry_i); try earliest first
    cand_t = {rel}
    for _ in range(min(SWAP_TIME_SAMPLES, span)):
        cand_t.add(rel + rng.randrange(span))
    for t in sorted(cand_t):
        ...
        for oi in orients:
            feas, cx0, cy0, occ_fp, near = raster.scan_scoped(
                bay_id, actives, i, oi, want_near=True)
            # only near-miss anchors (i ALMOST fits) name a small blocker set;
            # exact fits (feas) need no ejection and are the plain-tardy regime.
            if near is None or not near.any():
                continue
            cells = _order_cells(raster, near, cx0, cy0, i, oi,
                                 raster.W[bay_id], occ_fp, True, None)
            for (x, y) in cells[:SWAP_POS_CAP]:
                nb = _mkblock(i, blk, x, y, oi)
                blockers = _blockers_at(bay, relx, nb, t, exit_t)
                if not (1 <= len(blockers) <= BLOCKER_CAP):
                    continue
                # every blocker must have due-slack: removing and reinserting it
                # later is unlikely to create fresh tardiness.
                if any(blocks_data[bj]["due_date"] - cur[bj]["exit_time"] < 0
                       for bj in blockers):
                    continue
                removed = set(blockers)
                removed.add(i)
                return removed, [i] + sorted(blockers)
    return None
...
def _improve(prob_info, assignments, bays, bay_u, w1, w2, w3, deadline, forced,
    ...
    while time.time() < deadline:
        ...
        swap = False         # v28b: resident<->queued swap (i first, blockers after)
        ...
        if restart:
            ...
            if removed:
                ...
                if removed is None:
                    removed = _destroy_window(cur, blocks_data, rng)
        elif (raster is not None and best_tardy > 0
              and rounds % SWAP_EVERY == 0):
            # v28b: RESIDENT<->QUEUED 2-EXCHANGE. Fires only when tardy blocks
            # exist (best_tardy > 0) and on a fixed rounds cadence (no rng in the
            # gate) -> byte-neutral on zero-tardy instances: this branch is never
            # entered, so the existing modes' rng streams below are untouched. rng
            # is consumed ONLY inside _destroy_blocker_swap (a fired-branch draw).
            # On a miss it falls through to the classic tardy destroy for the round.
            res = _destroy_blocker_swap(cur, blocks_data, bays, raster, rng)
            if res is not None:
                removed, priority = res
                swap = True
            else:
                k = min(6, max(2, n // 40))
                removed = _destroy_tardy(cur, blocks_data, k, rng)
        ...
        if (spread or swap) and priority:
            # v28b: swap reuses the explicit-priority plumbing -- i FIRST so it
            # grabs the freed earlier slot, ejected blockers reinsert after. bay
            # order stays default (bo=None) so i is placed at its earliest feasible
            # slot rather than the load-ascending spread order.
...
def _run_strategy(wid, prob_info, timelimit, t_start, push, inbox=None):
    ...
    if wid == 3 and raster is not None and forced and len(blocks_data) >= 250:
        ...
        if nm_elig:
            try:
                # v26b: the FIRST seed build becomes a DEEP-NESTLE near_k=32
                # build (nearmiss=40), feeding a deep-ish near_k basin to this
                # giant specialist's obj-gated repack_every=2 + xbay polish.
                # Floor-safe: improve is min-wins and the ovh second seed stays
                # byte-exact v25. raster.near_k restored immediately after and
                # in the except so it can never leak.
                raster.near_k = 32
                _o, a = dispatch(1.0, 0.5, alpha=1.0, beam=True, nearmiss=40,
                                 dl=t_start + 0.22 * window)
                raster.near_k = 3
                try:
