...
# myalgorithm_jv12.py -- jv12 = jv9 + GIANT MERGE-TAIL (jiyun).
# The merge-tail recombination (campaign's only positive-yield mechanism,
# -383k non-forced) is UNGATED for forced GIANTS (n>=250 = {27,37,38,39}):
# the capacity campaign gave their 8 workers real inter-basin diversity
# (27 +/-425k), and ~49s of parent "remaining" already exists after
# search_deadline -- so recombine the 8 diverse solutions (official-verified,
# min-wins, wider 6s safety for the 250-block feasibility check). Non-giant
# forced stay byte-identical. OGC_GIANT_MERGE=0 disables. See heuristic_jv12.
# ---------------------------------------------------------------------------
...
def _merge_tail(prob_info, bays, bay_u, w1, w2, w3, cands, winner_assign,
    ...
    # jv12 GIANT MERGE: the merge tail (the campaign's ONLY positive-yield
    # mechanism, -383k on non-forced) has never fired on the giants -- the
    # `_is_forced` gate blocked it. But the capacity campaign (jv7-jv9) gave
    # the 8 giant workers real inter-basin diversity (27 spread +/-425k
    # between draws), which is exactly what the recombination CP-SAT exploits.
    # And ~49s of parent "remaining" already exists after search_deadline on a
    # 750s run -- no worker-time theft. So ALLOW forced GIANTS (n>=250):
    # recombine their 8 diverse polished solutions (pick per-block best,
    # feasibility via incompat pairs + replay repair, official-verified,
    # min-wins). Non-giant forced (32/40/mid) stay blocked (low diversity,
    # lottery-sensitive). OGC_GIANT_MERGE=0 disables for A/B.
    _forced = _is_forced(prob_info, bays)
    _giant = _forced and n_blocks >= 250 and \
        _os.environ.get("OGC_GIANT_MERGE", "1") != "0"
    if (not _HAVE_NUMPY or remaining < MERGE_MIN or k < 3
            or (_forced and not _giant)):
        _emit(False, 0.0)
    ...
    if len(pool) != n_blocks:
        ...
        return winner_assign
    try:
        # CP-SAT budget = remaining - safety (covers precompute + replay
        # repair + final official verify). merge_hard_stop bakes it in, so the
        # solve deadline never blows the parent's abs_stop. jv12: giants get a
        # WIDER safety (6s) -- the final check_feasibility + _build_operations
        # run over 250 blocks (vs ~<100 non-forced), so the post-solve verify
        # needs more headroom to never overrun the hard time limit.
        safety = 6.0 if _giant else 2.5
...
def _algorithm_portfolio(prob_info, timelimit, t_start):
    ...
    # jv12: GIANT-MERGE flag. Giants (forced n>=250 = {27,37,38,39}) route
    # their feasible winner through _merge_tail to recombine the 8 diverse
    # worker solutions (the merge's own gate re-checks n>=250 + env). Non-
    # giant forced stay TOKEN-IDENTICAL to v25 (lottery-sensitive prob_32).
    _giant_merge = (_forced and len(prob_info["blocks"]) >= 250
                    and _os2.environ.get("OGC_GIANT_MERGE", "1") != "0")
    if _forced:
        # v33 FORCED PATH: TOKEN-IDENTICAL to v25's verify loop (no _merge_tail,
        # no dedup, no extra references) EXCEPT jv12 giants, which run the merge
        # on the feasible winner. The merge is official-verified + min-wins, so
        # a giant that doesn't improve returns byte-identical.
