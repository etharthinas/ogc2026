...
HMATCH_W = 12.0          # 30s sweep: 4.0 -> 12.0 (fragmentation term x3)
...
def _run_strategy(wid, prob_info, timelimit, t_start, push, inbox=None):
    ...
    if wid == 2 and raster is not None:
        ...
        if forced:
            ...
            if nm_elig:
                import os as _os30b
                if _os30b.environ.get("OGC_DEBUG"):
                    print(f"[w2 trio] rem={cap - time.time():.1f}s", flush=True)
                for _hm, _tf in ((True, False), (False, True), (True, True)):
                    ...
                    try:
                        ...
                        if _os30b.environ.get("OGC_DEBUG"):
                            print(f"[w2 trio] fired hm={_hm} tf={_tf} "
                                  f"rem={cap - time.time():.1f}s", flush=True)
                    except Exception:
                        break
        else:
            try:
    ...
    if (wid == 1 and raster is not None and forced and w1 >= 6000
        ...
        try:
            ...
            import os as _os30
            if _os30.environ.get("OGC_DEBUG"):
                print(f"[w1 trio] ji={ji} rem={tick_dl - time.time():.1f}s",
                      flush=True)
            for _hm, _tf in ((True, False), (False, True), (True, True)):
                if time.time() >= tick_dl:
                    break
                ...
                if _os30.environ.get("OGC_DEBUG"):
                    print(f"[w1 trio] fired hm={_hm} tf={_tf} "
                          f"rem={tick_dl - time.time():.1f}s", flush=True)
        except Exception:
            pass
        if cands:
