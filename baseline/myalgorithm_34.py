# myalgorithm_34.py  --  v34 = v33 + FORCED-INSTANCE OBJ2/OBJ3-ONLY TAIL PASS.
# =============================================================================
# v34. Byte-exact copy of v33 plus a post-race, obj1-FROZEN local-search tail
# that runs on FORCED instances ONLY, harvesting the ~7M of banked obj2/obj3
# mass (measured: prob_38 2.10M, prob_27 1.96M, prob_31 2.83M, prob_37 2.81M)
# that every historical lever ignored because they all chased obj1. On forced
# instances the merge gate is closed and the portfolio finishes ~35-45s before
# the deadline (measured prob_38 565/600s, prob_27 550/600s); that tail is free.
#   * HOOK: strictly AFTER the race (workers terminated, winner selected). Fires
#     inside the v33 FORCED verify branch, once a candidate passes the official
#     check_feasibility, when remaining = deadline - now >= 10s. On non-forced
#     instances the branch is byte-identical to v33 (merge tail untouched); the
#     tail never touches any pre-deadline / race-time code.
#   * MOVES (`_tail34_pass`): TIME-FROZEN relocations on the champion assignment.
#     Every move keeps a block's entry_time/exit_time EXACTLY -> obj1 is frozen
#     by construction (obj1 depends only on exit_time vs due_date). (a) OBJ3
#     RELOCATION: each off-preference block (penalty > 0, desc by penalty) is
#     tried into higher-preference bays; (b) OBJ2 REBALANCE: blocks in the bay
#     with the max normalized load are tried into less-loaded bays. Both use the
#     SAME exact accept test: the reassignment's exact internal-objective delta
#     w2*d_obj2 + w3*d_obj3 (imbal() recomputed exactly on the moved workload,
#     obj3 delta = prefs[cb]-prefs[tj]) must be strictly negative, and a feasible
#     time-frozen (x,y,orient) must exist in the target bay -- found with the
#     SAME primitives v25/_z3_relocate use: raster.scan_scoped + _order_cells +
#     the exact _can_place gate against the target bay's residents (the moved
#     block sits in its OLD bay during the scan, so "minus the block itself" is
#     automatic; removing it from the old bay only ever REDUCES obstacles there).
#     Greedy sweeps (phase A then B); stop when a full sweep accepts nothing or
#     the deadline (2.5s safety) is hit.
#   * SAFETY (three independent guards): (1) exact-objective accept -> internal
#     objective monotonically decreases; (2) obj1 recomputed before/after and the
#     pass is DISCARDED if obj1 changed at all (asserts the freeze); (3) the whole
#     mutated assignment is rebuilt to operations and re-run through the official
#     utils.check_feasibility -- the crane subtlety (a relocation perturbs BOTH
#     bays' entry/exit replay, caught only at official stage 5) is handled by
#     reverting to the pre-pass champion on any infeasibility. Returns the better
#     of {tail, champion} by OFFICIAL objective; a failed tail costs nothing.
#     OGC_DEBUG emits: [tail34] forced=<b> moves=<n> dObj3=<d> dObj2=<d> rem_start=<s>.
# =============================================================================
# (v33 header below, kept verbatim for provenance)
...
def _tail34_pass(prob_info, champ_assign, bays, bay_u, w1, w2, w3, deadline):
    """v34 FORCED obj2/obj3-only tail. TIME-FROZEN local relocations on the
    post-race champion: every move keeps the block's [entry, exit) interval, so
    obj1 is invariant by construction and only obj2/obj3 shift. Two greedy move
    families, both exact-objective-gated (accept iff w2*d_obj2 + w3*d_obj3 < 0):
      (a) OBJ3 RELOCATION -- off-preference blocks -> higher-preference bays;
      (b) OBJ2 REBALANCE   -- blocks in the max-normalized-load bay -> lighter
          bays.
    Candidate (x,y,orient) come from the scoped raster scan, exact-gated with
    _can_place against the target bay's residents (identical to _z3_relocate's
    proven placement primitive). Returns (new_assign, n_moves, d_obj2, d_obj3)
    where the deltas are exact component changes, or (None, 0, 0.0, 0.0) if
    nothing moved / prerequisites missing. The CALLER runs the official
    check_feasibility and obj1-freeze assertion; this routine only mutates a
    private copy and never changes any entry/exit time."""
    blocks_data = prob_info["blocks"]
    n_bays = len(bays)
    if n_bays < 2 or not _HAVE_NUMPY:
        return None, 0, 0.0, 0.0
    raster = _Raster(prob_info, bays)
    cur = {bi: dict(a) for bi, a in champ_assign.items()}
    sched, bay_loads = _rebuild_sched(cur, blocks_data, n_bays)
    ...
    _, base_o1, base_o2, base_o3 = _objective(cur, blocks_data, bays, bay_u,
                                              w1, w2, w3)
    moves = 0
    POS_CAP = 24                       # exact _can_place gates per (bay, orient)

    def try_relocate(bi, target_bays):
        """Attempt a TIME-FROZEN relocation of block bi into one of target_bays.
        Commits + returns True iff the exact internal objective strictly
        improves (obj1 provably unchanged: entry/exit are never touched)."""
        nonlocal moves
        a = cur[bi]
        blk = blocks_data[bi]
        cb = a["bay_id"]
        entry, exit_t = a["entry_time"], a["exit_time"]     # FROZEN
        wl = blk["workload"]
        prefs = blk["bay_preferences"]
        orients = _unique_orients(blk)
        im0 = imbal(bay_loads)
        for tj in target_bays:
            if tj == cb:
                continue
            bay = bays[tj]
            if not any(_orient_fits(blk, oi, bay) for oi in orients):
                continue
            # EXACT objective delta of the (cb -> tj) reassignment at the SAME
            # interval. obj1 is invariant (exit_t unchanged); obj2 recomputed on
            # the moved workload; obj3 delta = prefs[cb] - prefs[tj] (since the
            # per-block penalty is max(prefs) - prefs[bay]).
            loads2 = list(bay_loads); loads2[cb] -= wl; loads2[tj] += wl
            d_o2 = imbal(loads2) - im0
            d_o3 = prefs[cb] - prefs[tj]
            d_obj = w2 * d_o2 + w3 * d_o3
            if d_obj >= -1e-9:
                continue                        # move cannot pay for itself
            relx = _time_overlap_rel(sched[tj], entry, exit_t)
            actives = [(b.block_id, b.orient_idx, b.x, b.y)
                       for b, _a2, _e2 in relx]
            for oi in orients:
                if not _orient_fits(blk, oi, bay):
                    continue
                feas, cx0, cy0, occ_fp = raster.scan_scoped(tj, actives, bi, oi)
                if feas is None or not feas.any():
                    continue
                cells = _order_cells(raster, feas, cx0, cy0, bi, oi,
                                     raster.W[tj], occ_fp, True, None)
                ...
                for (x, y) in cells:
                    ...
                    if _can_place(bay, relx, nb, entry, exit_t):
                        # commit: strictly-improving, feasible, time-frozen.
                        sched[cb] = [it for it in sched[cb]
                                     if it[0].block_id != bi]
                        sched[tj].append((nb, entry, exit_t, nb.bounding_rect()))
                        bay_loads[cb] -= wl; bay_loads[tj] += wl
                        a["bay_id"] = tj
                        a["x"] = int(x); a["y"] = int(y)
                        a["orient_idx"] = oi
    ...
    try:
        while time.time() < deadline:
            sweep_moved = False
            # -- Phase A: OBJ3 relocation (off-preference blocks first) --------
            cand_a = []
            for bi, a in cur.items():
                prefs = blocks_data[bi]["bay_preferences"]
                pen = max(prefs) - prefs[a["bay_id"]]
                if pen > 0:
                    cand_a.append((pen, bi))
            ...
            for _, bi in cand_a:
                ...
                prefs = blocks_data[bi]["bay_preferences"]
                cb = cur[bi]["bay_id"]
                higher = sorted((j for j in range(n_bays) if prefs[j] > prefs[cb]),
                                key=lambda j: -prefs[j])
                if try_relocate(bi, higher):
                    sweep_moved = True
            # -- Phase B: OBJ2 rebalance (drain the heaviest normalized bay) ----
            if time.time() < deadline:
                nl = [bay_u[j] * bay_loads[j] for j in range(n_bays)]
                src = max(range(n_bays), key=lambda j: nl[j])
                lighter = sorted((j for j in range(n_bays) if j != src),
                                 key=lambda j: nl[j])
                cand_b = sorted((bi for bi, a in cur.items()
                                 if a["bay_id"] == src),
                                key=lambda bi: -blocks_data[bi]["workload"])
                for bi in cand_b:
                    if time.time() > deadline:
                        break
                    if try_relocate(bi, lighter):
                        sweep_moved = True
            if not sweep_moved:
                break
    except Exception:
    ...
    if moves == 0:
        return None, 0, 0.0, 0.0
    _, new_o1, new_o2, new_o3 = _objective(cur, blocks_data, bays, bay_u,
                                          w1, w2, w3)
    # obj1-FREEZE ASSERTION: time-frozen moves cannot change obj1; if they
    # somehow did, discard the whole pass (correctness over a speculative gain).
    if abs(new_o1 - base_o1) > 1e-9:
        return None, 0, 0.0, 0.0
    return cur, moves, (new_o2 - base_o2), (new_o3 - base_o3)
    ...
    if _forced:
        ...
        for _, assign in cands:
            ...
            try:
                ...
                continue
            if res["feasible"]:
                # v34 FORCED OBJ2/OBJ3-ONLY TAIL PASS. Strictly post-race and
                # post-winner-selection. Fires only with real spare time (the
                # measured forced-instance situation: workers early-terminate
                # ~35-45s before the deadline). obj1 is frozen by construction;
                # every accept is exact-objective-gated AND the mutated whole is
                # re-verified by the official check_feasibility below, so a
                # failed/regressive tail cannot degrade the champion `sol`.
                rem_start = t_start + timelimit - time.time()
                if rem_start >= 10.0:
                    try:
                        tail_dl = t_start + timelimit - 2.5   # 2.5s safety
                        t_assign, t_moves, t_do2, t_do3 = _tail34_pass(
                            prob_info, assign, bays, bay_u, w1, w2, w3, tail_dl)
                        if _dbg:
                            print(f"[tail34] forced=True moves={t_moves} "
                                  f"dObj3={t_do3:.6g} dObj2={t_do2:.6g} "
                                  f"rem_start={rem_start:.1f}", flush=True)
