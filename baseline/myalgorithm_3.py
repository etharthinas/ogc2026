...
# =============================================================================
# See heuristics/heuristic_3.md. Reuses v2's verified feasibility/placement core
...
def _improve(prob_info, assignments, bays, bay_u, w1, w2, w3, deadline, forced):
    """Destroy worst-tardy + early-occupying neighbors, repair EDD-earliest.
    Compares by the cheap internal objective; accepts only strict improvements.
    The caller verifies the final result with the official checker, so this stays
    fast (no per-round check_feasibility) yet monotone w.r.t. internal objective
    (which mirrors the checker for feasible-by-construction solutions)."""
    blocks_data = prob_info["blocks"]
    n_bays = len(bays)
...
def algorithm(prob_info, timelimit=60):
    ...
    # Strategy priority. A thorough construction can consume the whole budget on
    # large instances, so order matters and later strategies run only on leftover
    # time. EDD-earliest is primary on forced/congested instances (kills the
    # avoidable tardiness that dominates the objective); congestion+zero-slot
    # Pass A is primary on uncongested instances (already near floor in v2).
    edd = _edd_order(blocks_data)
    cong = _congestion_order(blocks_data)
    if forced:
        strategies = [(edd, True), (cong, False)]
    else:
        strategies = [(cong, False), (edd, True)]

    for order, fc in strategies:
    ...
    # Multi-start EDD-jitter perturbations on leftover time (cheap; the
    # construction self-throttles to the deadline).
    ...
    # Safe monotone improver on the current best candidate.
