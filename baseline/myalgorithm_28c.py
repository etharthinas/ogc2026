# myalgorithm_28c.py  --  v28c = v28 substrate + ROLLOUT ADMISSION SCORING in the
#   W3 giant specialist's FIRST seed build ONLY (heuristic_28.md Improvement C).
#   Verdict (ledger_27_diag): during giant bursts >=99.7% of queued-block-moments
#   have NO feasible placement and 100% of tardiness is entry-queue delay -- the
#   lever is SEQUENCING (which queued block is admitted when a slot opens), not
#   packing. So at DEEP-QUEUE events (queue depth >= ROLLOUT_QDEPTH=8) during the
#   giant's first seed construction, the top-B=3 ATC candidates are each scored by
#   a short greedy ROLLOUT (admit the candidate, then greedy-ATC continuation for
#   H=15 events on a CLONED raster, projected obj1 = placed tardiness + LB
#   tardiness of the still-queued set); the candidate with the lowest projected
#   obj1 is admitted first, then the rest of the queue greedily in ATC order. This
#   replaces beam_admit's myopic (area,cost) choice at those events with a
#   downstream-queue-aware one.
#   SCOPE: gated behind a new `rollout` flag threaded dispatch -> _dispatch_construct,
#   default False everywhere -> ALL other calls (W1/W0/W2, both giant seeds bar the
#   first, every non-giant) are BYTE-IDENTICAL to v28. Only the W3-giant first-seed
#   dispatch (nm+beam, near_k=32) passes rollout=True. 26b proved this slot is free
#   real estate: the improve is obj-gated / min-wins, so worst case the rollout seed
#   never beats the inbox leader and contributes/displaces nothing (byte-flat floor).
#   The rollout uses raster.scan feasibility only (SOUND: a scan-clear anchor is
#   provably collision+crane clear, so no _can_place needed) on a snapshot/clone of
#   the raster occupancy that is restored in a finally; it consumes NO rng (pure
#   greedy ATC + contact BL) so main streams are undisplaced; a `deadline - margin`
#   guard skips the rollout once the seed build nears its 0.22w deadline.
# ---------------------------------------------------------------------------
...
# v28c ROLLOUT ADMISSION constants (heuristic_28.md Improvement C). Consulted
# ONLY when _dispatch_construct is called with rollout=True (the W3-giant first
# seed build); every other call ignores them entirely -> byte-identical to v28.
ROLLOUT_B = 3            # top-B ATC candidates scored per deep-queue event
ROLLOUT_H = 15           # greedy-continuation horizon (distinct events) per rollout
ROLLOUT_QDEPTH = 8       # queue depth at which a deep-queue event triggers rollout
ROLLOUT_MARGIN = 1.0     # secs of build deadline reserved -> skip rollout past it
...
def _dispatch_construct(prob_info, bays, bay_u, w1, w2, w3, deadline, raster,
    ...
    # v28c ROLLOUT ADMISSION helpers (active only when rollout=True). --------
    def _rollout_place(bi, t):
        """Cheap SOUND raster-only placement for the rollout simulation: first
        bay (by bay_score) with a scan-clear anchor, best cell by the same
        contact ordering the real first-seed build uses (score_pos=True, no rng,
        no ovh). scan-clear => provably collision+crane free, so no _can_place is
        needed. Returns a place tuple or None (near-miss/_can_place recovery is
        deliberately skipped -- the rollout is a conservative ranking proxy)."""
        blk = blocks_data[bi]
        exit_t = t + procs[bi]
        for bay_id in sorted(range(n_bays), key=lambda j: bay_score(bi, j, t)):
            bay = bays[bay_id]
            occ_fp = raster.footprint(bay_id)
            for oi in orients_of[bi]:
                if not _orient_fits(blk, oi, bay):
                    continue
                feas, cx0, cy0 = raster.scan(bay_id, bi, oi)
                ...
                x, y = cells[0]
                return (bay_id, int(x), int(y), oi, t, exit_t)
        return None

    def _rollout_obj1(cand_first, t0):
        """Projected obj1 of admitting `cand_first` at t0 then continuing the
        greedy ATC policy for ROLLOUT_H events, on a CLONE of the raster occupancy
        (snapshot/restore in finally). obj1 = sum tardiness of rollout-placed
        blocks + lower-bound tardiness max(0, t_now+proc-due) of the still-queued
        set. Real future exits (exits_at) are replayed so residents free space on
        schedule. Consumes no rng (atc's jitter rng is None in this build)."""
        save_occ = [raster.occ[j] for j in range(n_bays)]
        save_ver = list(raster.ver)
        try:
            for j in range(n_bays):
                raster.occ[j] = {l: g.copy() for l, g in save_occ[j].items()}
                raster.ver[j] += 1
                raster._scan[j] = dict(); raster._fp[j] = None
                raster._uni[j] = None; raster._nearc[j] = dict()
            qset = set(queue)
            rp2 = rp
            rex = {}                         # exit time -> [(bay,bi,oi,x,y), ...]
            for tt, v in exits_at.items():
                rex.setdefault(int(tt), []).extend(v)
            evh = list(rex.keys())
            for k in range(rp2, n):
                evh.append(rels[rel_sorted[k]])
            heapq.heapify(evh)
            accrued = 0.0
            cur_t = t0
            last_t = t0
            ...
            first_event = True
            while True:
                ex_ev = rex.pop(cur_t, None)
                if ex_ev:
                    for (bay_id, bi, oi, x, y) in ex_ev:
                        raster.remove(bay_id, bi, oi, x, y)
                while rp2 < n and rels[rel_sorted[rp2]] <= cur_t:
                    qset.add(rel_sorted[rp2]); rp2 += 1
                if qset:
                    order2 = sorted(qset, key=lambda bi: -atc(bi, cur_t))
                    if first_event and cand_first in qset:
                        order2 = [cand_first] + [b for b in order2
                                                 if b != cand_first]
                    for bi in order2:
                        if targets is not None and cur_t < targets[bi]:
                            continue
                        place = _rollout_place(bi, cur_t)
                        if place is None:
                            continue
                        bay_id, x, y, oi, en, ex = place
                        raster.add(bay_id, bi, oi, x, y)
                        rex.setdefault(ex, []).append((bay_id, bi, oi, x, y))
                        heapq.heappush(evh, ex)
                        accrued += max(0, ex - dues[bi])
                        qset.discard(bi)
                first_event = False
                last_t = cur_t
                events_done += 1
                if events_done > ROLLOUT_H:
                    break
                nxt = None
                while evh:
                    ct = heapq.heappop(evh)
                    if ct > cur_t:
                        nxt = ct; break
                if nxt is None:
                    break
            ...
            lb = 0.0
            for bi in qset:
                lb += max(0, last_t + procs[bi] - dues[bi])
            return accrued + lb
        finally:
            for j in range(n_bays):
                raster.occ[j] = save_occ[j]
                raster.ver[j] = save_ver[j]
                raster._scan[j] = dict(); raster._fp[j] = None
                raster._uni[j] = None; raster._nearc[j] = dict()

    def _rollout_pick(cands, t):
        """Return the candidate (from ATC-top-B `cands`) with the lowest projected
        rollout obj1; ties / failures keep the ATC head (cands[0])."""
        best_bi = cands[0]
        best_obj = None
        for c in cands:
            try:
    ...
    while heap:
        ...
        if (rollout and len(ordered) >= ROLLOUT_QDEPTH
                and time.time() <= deadline - ROLLOUT_MARGIN):
            # v28c: SEQUENCING lever. This is a deep-queue event -- rollout-score
            # the top-B ATC candidates and admit the one with the lowest
            # projected downstream obj1 FIRST, then fill the rest greedily in ATC
            # order. Replaces beam_admit's myopic (area,cost) choice at exactly
            # the events the diagnostic flagged (entry-queue delay is 100% of
            # obj1). rollout=True is set only by the W3-giant first seed build.
            head = _rollout_pick(ordered[:ROLLOUT_B], t)
            for bi in ([head] + [b for b in ordered if b != head]):
                if time.time() > deadline:
                    break
                if targets is not None and t < targets[bi]:
                    continue
...
def _run_strategy(wid, prob_info, timelimit, t_start, push, inbox=None):
    ...
    def dispatch(kappa, gamma, drng=None, alpha=0.0, score_pos=True,
                 tspec=None, beam=False, dl=None, nearmiss=0, rollout=False):
        raster.reset()
        a = _dispatch_construct(prob_info, bays, bay_u, w1, w2, w3,
                                ...
                                nearmiss=nearmiss, rollout=rollout)
        o = iobj(a)
        push(o, a)
        return o, a
    ...
    if wid == 3 and raster is not None and forced and len(blocks_data) >= 250:
        ...
        if nm_elig:
            try:
                ...
                # v28c: this first seed build ALSO enables ROLLOUT admission
                # scoring (rollout=True) -- the ONLY call in the program that
                # does. The sequencing rollout fires at deep-queue events during
                # this construction (see _dispatch_construct); every other
                # dispatch/build is byte-identical to v28. Still min-wins /
                # obj-gated, so worst case it contributes nothing (26b floor).
                raster.near_k = 32
                _o, a = dispatch(1.0, 0.5, alpha=1.0, beam=True, nearmiss=40,
                                 dl=t_start + 0.22 * window, rollout=True)
                raster.near_k = 3
                try:
