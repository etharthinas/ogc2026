# myalgorithm_31.py  --  v31a = v25 + EPOCH RESET ON CONVERGENCE (heuristic_31
# Improvement A). Portfolio worker-level: when an armed improve stream has
# stalled on obj1 for EPOCH_AFTER (=120) rounds AND >= EPOCH_MIN_REM (=90s) of
# worker budget remain, `_improve` breaks early (keeping its monotone best) and
# raises the process-local _EPOCH_FIRED flag; the worker then re-runs its ENTIRE
# construction phase with a shifted seed base (all rng seeds + 7777; fresh drng
# instances via the ep_drng fallback) on the REMAINING budget (bud_t0/bud_win),
# polishes the new epoch's champion, and every champion is pushed through the
# obj-gated inbox (min-wins -> floor-safe). Epoch 1 IS v25: the gate is
# `epoch_dl is not None and forced and best_tardy>0 and since_o1>=120 and
# rem>=90` -- prob_1 (non-forced, zero-tardy) and the round-starved giants
# (since_o1<=16, 28f telemetry) can NEVER reach it, so both stay byte-identical
# by construction; when the gate never fires the whole mechanism is inert (the
# O(1) counter check consumes no rng). Armed: W1 nm_elig, W2 raster, W3 general
# dispatcher (the branches that improve-spin on the 26/27/31/33 converged
# class). W0 deliberately NOT armed (see heuristic_31 report, deviation d).
# ---------------------------------------------------------------------------
...
# v31a EPOCH RESET: process-local flag raised by _improve when an ARMED improve
# stream detects early convergence with budget to spare (see header). A 1-elem
# list so nested closures mutate it without a `global` decl; each portfolio
# worker is its own ctx.Process, so this is never shared across workers. The
# worker resets it to False before epoch 0 and reads it after epoch 0's polish.
_EPOCH_FIRED = [False]
EPOCH_AFTER = 120     # since_o1 rounds of obj1 stall before a reset is allowed
EPOCH_MIN_REM = 90.0  # worker-budget seconds that must remain to bother resetting
...
def _improve(prob_info, assignments, bays, bay_u, w1, w2, w3, deadline, forced,
    ...
    while time.time() < deadline:
        ...
        if best_tardy == 0 and since_best > 40:
            break
        # v31a EPOCH RESET gate (O(1), O(1) counter reads only; consumes no rng).
        # Fires ONLY on an ARMED stream (epoch_dl set by the worker) that has
        # genuinely converged with budget to spare: forced + still-tardy +
        # obj1 stalled EPOCH_AFTER rounds + >= EPOCH_MIN_REM worker-budget left.
        # `since_o1 >= EPOCH_AFTER` short-circuits before the time() call, so the
        # pre-convergence hot loop pays only 3 boolean reads. Non-armed callers
        # (epoch_dl is None: the v9 replica, every non-target instance) skip it
        # entirely -> byte-identical. prob_1 (not forced / zero-tardy) and the
        # round-starved giants (since_o1 <= 16, 28f telemetry) never satisfy it.
        # Break keeps the monotone `best`; the worker sees the flag and re-epochs.
        if (epoch_dl is not None and forced and best_tardy > 0
                and since_o1 >= EPOCH_AFTER
                and epoch_dl - time.time() >= EPOCH_MIN_REM):
            _EPOCH_FIRED[0] = True
            break
        rounds += 1
...
def _run_strategy(wid, prob_info, timelimit, t_start, push, inbox=None):
    ...
    # v31a EPOCH RESET nonlocals (read by the dispatch/ticket/polish closures,
    # written by the armed branches). Initialized to the epoch-0 identity:
    #   seed_offset = 0   -> every rng seed is v25's (byte-identical)
    #   ep_drng     = None-> dispatch/ticket keep their v25 (deterministic) rng
    #   bud_t0/win  = t_start/window -> polish + lottery caps use v25's budget clock
    # Epoch 1 (a reset) sets seed_offset=7777, a fresh ep_drng, and shifts
    # bud_t0/bud_win to (now, remaining budget). t_start passed to _construct/
    # _dispatch_construct stays the real scheduling origin and is never touched.
    seed_offset = 0
    ep_drng = None
    bud_t0 = t_start
    bud_win = window
    ...
    def improve(assign, seed, until=None, repack_every=0, win_scale=2.0,
                xbay=None, nearmiss=0, epoch=False):
        ...
        # v31a: `epoch=True` (epoch-0 of an armed worker) hands _improve the WHOLE
        # worker deadline as the epoch-reset budget reference; epoch=False (all
        # non-armed callers, and every epoch-1 re-run) passes None -> gate inert.
        ...
        r, o = _improve(prob_info, assign, bays, bay_u, w1, w2, w3,
                        ...
                        nearmiss=nearmiss,
                        epoch_dl=(deadline if epoch else None))
        push(o, r, force=True)
        return o, r
    ...
    def dispatch(kappa, gamma, drng=None, alpha=0.0, score_pos=True,
        ...
        # v31a: on an epoch reset (ep_drng set), calls that carry NO explicit
        # rng adopt the shifted stream so the rebuilt lottery samples a fresh
        # basin. ep_drng is None in epoch 0 and for every non-armed worker, so
        # this is byte-inert there (drng stays None -> v25's deterministic build).
        if drng is None:
            drng = ep_drng
        raster.reset()
    ...
    def polish(assign, seed, repack_every=0, alts=None, nearmiss=0, epoch=False):
        ...
        v31a: `epoch=True` arms every improve stream below for the epoch-reset
        gate; on a fire the stream returns early and each stage bails (the worker
        starts epoch 1). All budget-clock references use bud_t0/bud_win, which
        equal t_start/window in epoch 0 and for non-armed workers (byte-exact),
        and are shifted to (now, remaining) for an epoch-1 re-run."""
        z3_at = deadline - (min(12.0, 0.05 * bud_win) if raster is not None
                            else 0.0)
        cp_at = bud_t0 + 0.62 * bud_win
        ...
        def cycles(specs):
            ...
            for cs, sc, cyc_dl, xb, ai in specs:
                ...
                o_, r_ = improve(src, cs, until=cyc_dl,
                                 ...
                                 xbay=xb, nearmiss=nearmiss, epoch=epoch)
                if o_ < ob:
                    base, ob = r_, o_
                if _EPOCH_FIRED[0]:
                    return   # bail this cycle set; worker starts epoch 1

        if giant:
            # -- EXACT v14 polish (see docstring) ------------------------------
            o1, r1 = improve(assign, seed, until=cp_at,
                             repack_every=repack_every, win_scale=2.0,
                             xbay=False, nearmiss=nearmiss, epoch=epoch)
            base, ob = r1, o1
            if _EPOCH_FIRED[0]:
                return
            try:
            ...
            if rc is not None:
                ...
                if oc < ob:
                    base, ob = rc, oc
            mid2 = bud_t0 + 0.85 * bud_win
            cycles([(seed + 1, 1.0, mid2, True, 0),
                    (seed + 2, 3.0, z3_at, True, 1)])
        # v31a: an epoch-reset fire already banked base via push(on_best); skip
        # the tardiness-neutral z3 endgame here (epoch 1 runs its own) and hand
        # control back to the worker.
        if (raster is not None and not _EPOCH_FIRED[0]
    ...
    if wid == 2 and raster is not None:
        # v31a EPOCH RESET wrapper. Epoch 0 == v25 exactly (seed_offset 0,
        # ep_drng None, bud clock = t_start/window). If epoch 0's armed polish
        # trips the reset gate (only reachable on the forced converged class),
        # epoch 1 re-runs the WHOLE lottery+polish with +7777 seeds, a fresh
        # ep_drng (which the deterministic W2 dispatches adopt for a new basin)
        # and the remaining budget; every champion is pushed (min-wins).
        _EPOCH_FIRED[0] = False
        for epoch in (0, 1):
          if epoch == 1:
            bud_t0 = time.time(); bud_win = deadline - bud_t0
            seed_offset = 7777
            ep_drng = random.Random(6363 + seed_offset)
            # The epoch-0 break-decision already consumed the fired flag; clear
            # it now so epoch 1's (unarmed) polish is NOT tripped by the stale
            # signal -- epoch 1 must spend the whole remaining budget descending.
            _EPOCH_FIRED[0] = False
          cands = []
          if forced:
          ...
          if cands:
            ...
            polish(min(cands, key=lambda c: c[0])[1], seed=2222 + seed_offset,
                   repack_every=3, alts=pick_alts(cands), epoch=(epoch == 0))
            ...
            try:
                ...
                improve(a, seed=2222 + seed_offset, repack_every=3,
                        epoch=(epoch == 0))
            except Exception:
                pass
          if epoch == 0 and not _EPOCH_FIRED[0]:
            break
        return
    ...
    if wid == 3 and raster is not None:
        # v31a EPOCH RESET wrapper. drng 9099 is the whole-lottery jitter seed,
        # so +seed_offset in epoch 1 re-rolls EVERY ticket -> a genuinely fresh
        # construction basin (the strongest of the three armed workers). Epoch 0
        # (seed_offset 0, bud clock t_start/window) is byte-exact v25 -- and this
        # branch runs prob_1 too (non-forced): the reset gate needs forced+tardy
        # so it can never fire there, leaving prob_1's W3 lottery byte-identical.
        _EPOCH_FIRED[0] = False
        for epoch in (0, 1):
          if epoch == 1:
            bud_t0 = time.time(); bud_win = deadline - bud_t0
            ...
            # signal -- epoch 1 must spend the whole remaining budget descending.
            _EPOCH_FIRED[0] = False
          drng = random.Random(9099 + seed_offset)
          cands = []
          cap = bud_t0 + 0.5 * bud_win
        ...
        # (measured on prob_35, overload 0.621: 2,011,342 -> 1,346,898).
          ...
          if cands:
              ...
              polish(min(cands, key=lambda c: c[0])[1], seed=3333 + seed_offset,
                     repack_every=4, alts=pick_alts(cands), epoch=(epoch == 0))
          elif time.time() < deadline:  # fallback
              try:
                  _, a = build(_area_order(blocks_data) if forced
                               else _edd_order(blocks_data), forced)
                  improve(a, seed=3333 + seed_offset, repack_every=4,
                          epoch=(epoch == 0))
              except Exception:
                  pass
          if epoch == 0 and not _EPOCH_FIRED[0]:
    ...
    if (wid == 1 and raster is not None and forced and w1 >= 6000
            and overload > 0.65):
        # v31a EPOCH RESET wrapper. Epoch 1 re-rolls the ticket lottery: _jrng
        # is shifted +7777 AND the deterministic tickets adopt ep_drng (the
        # `if drng is None: drng = ep_drng` fallback in ticket()), so the whole
        # rotation samples fresh basins. Epoch 0 (seed_offset 0, ep_drng None,
        # bud clock t_start/window) is byte-exact v25 -- including the giants
        # 38/39, which share this branch but can never fire the round-starved
        # gate. If no cands, we fall through to the legacy W1 path exactly as v25.
        _EPOCH_FIRED[0] = False
        _w1_ok = False
        for epoch in (0, 1):
          if epoch == 1:
          ...
          cands = []
          try:
            tick_dl = bud_t0 + 0.55 * bud_win

            def ticket(tg, bm, nm, kap, al, drng=None, mpc=False,
                       ovh=False, nk=3):
                if drng is None:      # v31a: epoch-1 fresh basin (None in epoch 0)
                    drng = ep_drng
                raster.reset()
                raster.near_k = nk
            ...
            for kap, al, nk in ((0.5, 0.5, 24), (2.0, 0.5, 16),
                ...
                if time.time() >= bud_t0 + 0.42 * bud_win:
                    break
                ticket(None, True, nk + 8, kap, al, nk=nk)
            for kap, al in base_cfgs:
                if time.time() >= bud_t0 + 0.5 * bud_win:
                    break
            ...
            _jrng = random.Random(2121 + seed_offset)
            ji = 0
            ...
            pass
          if cands:
            polish(min(cands, key=lambda c: c[0])[1], seed=2077 + seed_offset,
                   repack_every=3, alts=pick_alts(cands), nearmiss=8,
                   epoch=(epoch == 0))
            _w1_ok = True
          if epoch == 0 and (not cands or not _EPOCH_FIRED[0]):
            break
        if _w1_ok:
            return
