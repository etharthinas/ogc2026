# myalgorithm_28f.py -- 28a CALIBRATION variant (spot prob_27 only). 28a @600s
#   was byte-flat on {38,27,39}; OGC_DEBUG showed prob_38 improve slices are
#   round-starved (since_o1 <= 16, gate 75 unreachable) while prob_27 slices
#   reach 76-127 rounds and restarts FIRED 1-2x but always lost. This variant
#   attacks fire-and-lose on 27-class instances only:
#     - RESTART_AFTER 60 -> 30, RESTART_EVERY 25 -> 15 (fires at 30,45,60,...)
#     - every 2nd fire rebuilds in volume-normalized-ATC order (due asc, then
#       min-footprint-area x processing asc; NO rng consumed) -- the variant
#       heuristic_28.md planned but 28a never implemented
#     - n_rebal counter actually incremented (was declared+printed only)
# ---------------------------------------------------------------------------
...
def _improve(prob_info, assignments, bays, bay_u, w1, w2, w3, deadline, forced,
    ...
    RESTART_AFTER = 30    # 28f: earlier gate (fires at since_o1=30,45,60..)
    RESTART_EVERY = 15    # 28f: denser cadence once past RESTART_AFTER
    ...
    while time.time() < deadline:
        ...
        if spread and priority:
            ...
            # 28f: every 2nd fire uses volume-normalized-ATC order instead (due
            # asc, then footprint-area x processing asc) -- the tardiness-
            # sequencing bias; deterministic, no rng consumed on those fires.
            if n_restart % 2 == 0:
                rem_order = sorted(
                    removed,
