# myalgorithm_jv10.py  --  jv10 = jv9 + SEED-DIVERSITY RACER + 2ND DEAF.
# W7 on nm_elig forced non-giants = zone/drain raw-frontier builds polished
# under fresh trajectories (r5/r6 deaths were single-trajectory). Giants:
# wid 5 goes deaf too (2 independent-basin + 2 leader-following replicas).
# See heuristics/heuristic_jv10.md.
# ---------------------------------------------------------------------------
...
def _run_strategy(wid, prob_info, timelimit, t_start, push, inbox=None):
    ...
    if wid >= 4:
        ...
        if forced and len(blocks_data) >= 250:
            ...
            # jv10: wid 5 goes deaf too (independence is the candidate active
            # ingredient behind 27's repeated yield; 2 deaf + 2 followers).
            if wid in (5, 6):
                inbox = None
            wid = 3
        else:
            # jv10: wid 7 = SEED-DIVERSITY RACER on the nm_elig forced
            # non-giants -- polishes the ZONE/DRAIN raw-frontier builds
            # (probe-proven -1.2M/-0.5M raw on 31/39) under a fresh
            # trajectory. Their r5/r6 portfolio deaths were single-
            # trajectory; the capacity law says trajectories decide.
            sdrace = wid == 7
    ...
    if (wid == 1 and raster is not None and forced and w1 >= 6000
        ...
        try:
            ...
            def ticket(tg, bm, nm, kap, al, drng=None, mpc=False,
                       ovh=False, nk=3, zone=0, drain=0, nmc=False):
                raster.reset()
                raster.near_k = nk
                a_ = _dispatch_construct(
                    ...
                    nearmiss=nm, mpc=mpc, ovh=ovh, zone=zone, drain=drain,
                    nm_compete=nmc)
                o_ = iobj(a_)
                push(o_, a_)
                cands.append((o_, a_))
            ...
            if sdrace:
                # jv10 SEED-DIVERSITY RACER: zone/drain constructions head
                # the window (deterministic pass), then jittered variations
                # around both families. Each ticket pushes its raw build
                # (floor-safe); the shared polish below runs the fresh
                # trajectory (seed 2077+soff) on the raw-min.
                _srng = random.Random(4242 + soff)
                zd_plan = (
                    dict(nmc=True, zone=24, nm=8, kap=0.5, al=0.5),
                    dict(drain=8, nk=32, nm=40, kap=1.0, al=0.0),
                    dict(nmc=True, zone=24, nm=8, kap=2.0, al=0.5),
                    dict(drain=8, nk=24, nm=32, kap=0.5, al=0.5),
                )
                zi = 0
                while time.time() < tick_dl and zi < 48:
                    c = zd_plan[zi % len(zd_plan)]
                    zi += 1
                    try:
                        ticket(None, True, c["nm"], c["kap"], c["al"],
                               drng=_srng if zi > len(zd_plan) else None,
                               nk=c.get("nk", 3), zone=c.get("zone", 0),
                               drain=c.get("drain", 0),
                               nmc=c.get("nmc", False))
                    except Exception:
                        pass
