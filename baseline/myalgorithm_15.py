# myalgorithm_15.py  --  v15 = v14 + CROSS-BAY Z3 GROUP RELOCATION + BASIN-
...
def _improve(prob_info, assignments, bays, bay_u, w1, w2, w3, deadline, forced,
             ...
             repack_every=0, xbay=False, repack_win_scale=2.0):
        ...
        if (repack_every > 0 and raster is not None and best_tardy > 0
            ...
            try:
                work = _repack_window(prob_info, cur, bays, bay_u, w1, w2, w3,
                                      ...
                                      mode=mode, win_scale=repack_win_scale)
            except Exception:
                work = None
...
def _run_strategy(wid, prob_info, timelimit, t_start, push, inbox=None):
    ...
    def improve(assign, seed, until=None, repack_every=0, win_scale=2.0,
        ...
        r, o = _improve(prob_info, assign, bays, bay_u, w1, w2, w3,
                        ...
                        xbay=xb, repack_win_scale=win_scale)
        push(o, r, force=True)
        return o, r
    ...
    def polish(assign, seed, repack_every=0, alts=None):
        ...
        giant = forced and len(blocks_data) >= 250
    ...
    if wid == 3 and raster is not None and forced and len(blocks_data) >= 250:
        try:
            ...
            return
        # v15 r2c: xbay=False restores the exact v14 giant-specialist draw
        # (the r1 xbay shot on prob_38 measured +53k; flat beats the re-roll).
        improve(a, seed=4444, repack_every=2, xbay=False)
        return
