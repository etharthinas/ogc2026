...
# non-giants / 4th light repack replica on giants. On giants the original
# wid-6 replica goes DEAF (inbox=None): independent-basin polish instead of
# leader-following. See heuristics/heuristic_jv9.md.
# ---------------------------------------------------------------------------
# (jv8 header, kept verbatim)
# myalgorithm_jv8.py  --  jv8 = jv7 + GIANT LIGHT-REPLICA WORKERS (jiyun).
# Giants (forced n>=250: {27,37,38,39}) now spawn W4..W6 as light repack-
# specialist replicas (W3-giant slot, soff-shifted rng, sparse seed + inbox-
# seeded improve; heavy nm_elig seed builds skipped when soff>0). Non-giant
# behavior is byte-identical to jv7. See heuristics/heuristic_jv8.md.
# ---------------------------------------------------------------------------
# (jv7 header, kept verbatim)
# myalgorithm_jv7.py  --  jv7 = jv6b + SEARCH-CAPACITY SCALING (jiyun).
# Three axes (heuristic_jv7.md):
#   A. Portfolio width: non-giant instances spawn up to 7 workers (was 4) on
#      8-core machines; W4..W6 = seed-diversified replicas of the W1/W3/W2
#      strategy slots (soff shifts every rng stream + polish seed; replicas of
#      the nm_elig plan-explorer skip the deterministic heads and spend the
#      whole window on jittered deep-nestle draws). Giants keep nw=4 + every
#      wid 0..3 path byte-identical (soff=0).
#   B. Merge-tail capacity: recombine CP-SAT gets all cores (parent runs it on
#      an idle machine; OGC_MERGE_WORKERS overrides), pool cap default 24->32
#      (OGC_MERGE_CAP) now that W4..W6 feed real new diversity.
#   C. (deferred) parent-as-searcher.
# ---------------------------------------------------------------------------
# (jv6 header, kept verbatim)
...
try:  # jv16: FFT correlation for the fine-raster scan
...
class _Raster:
    ...
    *touches* the closed unit square of local cell (cx0+j, cy0+i). Because the
    ...
    def __init__(self, prob_info, bays):
        ...
        # jv16 FINE RASTER: q subcells per unit axis. The unit-grid closed-cell
        # conservative mask dilates every fractional-vertex shape by up to a
        # full cell per edge -- measured true polygon density at tardiness-
        # forcing peaks is only 0.48-0.70 while the unit masks read "full".
        # q=4 shrinks the forced gap to <=1/4 unit; masks stay closed-cell
        # conservative (SOUND: never accept a real overlap), commits still
        # exact-gate via _can_place. q=1 keeps the legacy unit paths bit-exact.
        try:
            self.q = max(1, int(_z3os.environ.get("OGC_RASTER_Q", "4")))
        except Exception:
            self.q = 4
        self._mask = {}                       # (bi, oi) -> (mask, cx0, cy0) UNIT
        self._maskf = {}                      # (bi, oi) -> fine mask (nl, MH*q, MW*q)
        self._fpm = {}                        # (bi, oi) -> footprint-union mask
        self.occ = [dict() for _ in bays]     # bay -> {layer: int16 grid (fine)}
        ...
        self._SCOPED_CAP = 4096 if self.q == 1 else 512  # fine grids are q^2 bigger
    ...
    def fmask(self, bi, oi):
        """jv16: FINE mask (nl, MH*q, MW*q) uint8 on the 1/q subgrid, same
        (cx0, cy0) anchor mapping as mask(). Closed-subcell conservative:
        subcell marked iff the layer polygon touches it -- a strict superset
        of the footprint, so subcell-disjointness still PROVES no overlap.
        Built lazily per (bi, oi); q==1 aliases mask()."""
        if self.q == 1:
            return self.mask(bi, oi)
        key = (bi, oi)
        ...
        mask = _np.zeros((nl, MH, MW), dtype=_np.uint8)
        step = 1.0 / q
        cxs = cx0 + step * _np.arange(MW)
        cys = cy0 + step * _np.arange(MH)
        CX, CY = _np.meshgrid(cxs, cys)       # (MH, MW)
    ...
    def _apply(self, bay, bi, oi, x, y, sign):
        q = self.q
        mask, cx0, cy0 = self.fmask(bi, oi)
        nl, MH, MW = mask.shape
        r = (int(y) + cy0) * q; c = (int(x) + cx0) * q
        ...
        for l in range(nl):
            ...
            if g is None:
                if sign < 0:
                    continue
                g = _np.zeros((H, W), dtype=_np.int16)   # fine dims (H*q, W*q)
                occ[l] = g
    ...
    def footprint(self, bay):
        """Bool (H,W) UNIT grid: any layer occupied (compactness / util
        scoring). jv16: computed on the fine grid then any-pooled back to unit
        cells so every consumer keeps unit-grid semantics.
        ...
        fpf = _np.zeros((H * q, W * q), dtype=bool)
        for g in occ.values():
            fpf |= (g > 0)
        fp = (fpf.reshape(H, q, W, q).any(axis=(1, 3)) if q > 1 else fpf)
        self._fp[bay] = (self.ver[bay], fp)
        return fp

    def _unions(self, bay):
        # jv16: grids are fine (H*q, W*q); logic unchanged.
        cache = self._uni[bay]
        if cache is not None and cache[0] == self.ver[bay]:
            return cache[1]
        occ = self.occ[bay]
        H, W = self.H[bay] * self.q, self.W[bay] * self.q
        if not occ:
    ...
    def scan(self, bay, bi, oi):
        ...
        q = self.q
        mask, cx0, cy0 = self.fmask(bi, oi)
        nl, MH, MW = mask.shape                # fine dims when q > 1
        H, W = self.H[bay] * q, self.W[bay] * q
        if MH > H or MW > W:
            ...
            return None, cx0, cy0
        # anchors remain INTEGER positions: R,C in unit steps
        R = self.H[bay] - (MH // q if q > 1 else MH) + 1
        C = self.W[bay] - (MW // q if q > 1 else MW) + 1
        unions = self._unions(bay)
        maxL = len(unions) - 1
        total = _np.zeros((R, C), dtype=_np.float64 if q > 1 else _np.int32)
        for k in range(nl):
            if k > maxL:
            ...
            if q == 1:
                ...
                # jv16: FFT cross-correlation over the fine grid (cost is
                # independent of block size), then stride-sample the integer
                # anchors. corr[r, c] = sum(Vk[r:r+MH, c:c+MW] * mk).
                corr = _fftconv(Vk.astype(_np.float32),
                                   mk[::-1, ::-1].astype(_np.float32),
                                   mode='valid')
                total += corr[::q, ::q][:R, :C]
        feas = (total < 0.5)              # exact-zero test, FFT-noise safe
        self._scan[bay][(bi, oi)] = (self.ver[bay], feas, cx0, cy0)
        if self.near_enabled:
            # v20: near-miss anchors from the same count grid (byproduct).
            # jv16: near_k was tuned in UNIT cells -- scale to fine cells.
            nk = self.near_k * q * q
            self._nearc[bay][(bi, oi)] = (
                self.ver[bay], _np.logical_and(total > 0.5, total <= nk + 0.5))
        return feas, cx0, cy0
    ...
    def _scoped_union(self, bay, actives):
        ...
        for (b2, o2, x2, y2) in actives:
            ...
            if nl2 - 1 > maxL:
                maxL = nl2 - 1
        Hu, Wu = self.H[bay], self.W[bay]
        if maxL < 0:
            res = (-1, None, _np.zeros((Hu, Wu), dtype=bool))
        else:
            union_ge = [None] * (maxL + 1)
            cum = _np.zeros((H, W), dtype=bool)
            for l in range(maxL, -1, -1):
                ...
                if g is not None:
                    cum = cum | g
                union_ge[l] = cum
            # occ_fp consumers expect the UNIT grid -> any-pool back
            fp0 = union_ge[0]
            occ_fp_u = (fp0.reshape(Hu, q, Wu, q).any(axis=(1, 3))
                        if q > 1 else fp0)
            res = (maxL, union_ge, occ_fp_u)   # occ_fp = footprint of actives
    ...
    def scan_scoped(self, bay, actives, bi, oi, want_near=False):
        ...
        if want_near:
            nk = self.near_k * q * q
            return ((total < 0.5), cx0, cy0, occ_fp,
                    _np.logical_and(total > 0.5, total <= nk + 0.5))
        return (total < 0.5), cx0, cy0, occ_fp
...
def _run_strategy(wid, prob_info, timelimit, t_start, push, inbox=None):
    ...
    # jv7 CAPACITY SCALING: workers 4..6 are SEED-DIVERSIFIED replicas of the
    # W1..W3 strategy slots (W4->1, W5->3, W6->2 -- the plan-explorer and the
    # dispatcher lottery are the measured-winning families, so they get the
    # first extra cores). `soff` shifts every rng stream and polish seed so a
    # replica samples NEW construction jitter and NEW polish trajectories;
    # wid 0..3 keep soff=0 and remain byte-identical to jv6. min-wins + the
    # official best-first verify make every extra draw floor-safe.
    reserve = min(max(4.0, timelimit * 0.08), 12.0)
    ...
    # jv8: the remap needs `forced` -- on GIANTS every extra worker becomes a
    # LIGHT repack-specialist replica (W3-giant slot, ~1GB: sparse seed +
    # inbox-seeded improve with a shifted rng = an independent repack-polish
    # draw of the global leader; the soff>0 gate below skips W3's heavy
    # deterministic nm_elig seed builds so nothing is duplicated). Non-giants
    # keep jv7's W4->1, W5->3, W6->2 mapping. wid 0..3 stay byte-identical.
    soff = 0
    if wid >= 4:
        soff = 10007 * (wid - 3)
        if forced and len(blocks_data) >= 250:
            # jv9 DEAF replica: original wid 6 drops its inbox -- it polishes
            # its OWN sparse-seed basin to the deadline instead of adopting
            # the broadcast leader. All inbox-following replicas collapse
            # onto the leader's basin; this is the one stream doing true
            # independent-basin polish (27's -858k showed trajectories
            # matter; deaf = new trajectory AND new basin). Floor-safe.
            if wid == 6:
                inbox = None
            wid = 3
        else:
    ...
    if wid == 2 and raster is not None and not forced and overload > 0.44:
        ...
        while True:
            ...
            if time.time() >= cap or gi > 60:
                break
        if cands:
            polish_v13(min(cands, key=lambda c: c[0])[1], seed=3333 + soff)
        elif time.time() < deadline:
            try:
                _, a = build(_edd_order(blocks_data), forced)
                improve(a, seed=3333 + soff)
            except Exception:
                pass
        return
    ...
    if wid == 2 and raster is not None:
        ...
        if cands:
            ...
            polish(min(cands, key=lambda c: c[0])[1], seed=2222 + soff, repack_every=3,
            ...
            try:
                ...
                improve(a, seed=2222 + soff, repack_every=3)
            except Exception:
                pass
        return
    ...
    if wid == 3 and raster is not None and forced and len(blocks_data) >= 250:
        ...
        # jv8: replicas (soff>0) skip the heavy deterministic nm_elig seed
        # builds (they would duplicate W3's exact dispatches) and drop to the
        # cheap v14 sparse-seed path below -- the improver is inbox-seeded, so
        # each replica is an independent repack-polish draw of the global
        # leader with a shifted rng stream.
        if nm_elig and soff == 0:
            try:
                ...
                try:
                    ...
                    pass
                improve(a, seed=4444 + soff, repack_every=2, xbay=True, nearmiss=8)
                return
            except Exception:
        ...
        improve(a, seed=4444 + soff, repack_every=2, xbay=True)
        return
    ...
    if wid == 3 and raster is not None:
        ...
        if cands:
            ...
            polish(min(cands, key=lambda c: c[0])[1], seed=3333 + soff, repack_every=4,
                   alts=pick_alts(cands))
        elif time.time() < deadline:  # fallback
            try:
                _, a = build(_area_order(blocks_data) if forced
                             else _edd_order(blocks_data), forced)
                improve(a, seed=3333 + soff, repack_every=4)
            except Exception:
                pass
        return
    ...
    if (wid == 1 and raster is not None and forced and w1 >= 6000
        ...
        try:
            ...
            # jv7: replicas (soff>0) SKIP the deterministic head tickets --
            # re-running them would reproduce W1's exact raw builds and waste
            # the extra core. Replicas spend the whole ticket window on
            # jittered draws of the winning family instead (jitter x deep-
            # nestle nk rotation below), sampling basins W1 never visits.
            if soff == 0:
            ...
            for kap, al in (((0.5, 0.5), (2.0, 0.5)) if soff == 0 else ()):
            ...
            _jrng = random.Random(2121 + soff)
            ji = 0
            # jv7 replicas: full-window jitter, rotating the deep-nestle nk
            # family through the jittered draws (W1's head showed nk raw-
            # dominance; the jitter samples AROUND those anchors).
            _jcap = 24 if soff == 0 else 96
            _nks = (3,) if soff == 0 else (3, 24, 32)
            while time.time() < tick_dl and ji < _jcap:
                kap, al = base_cfgs[ji % len(base_cfgs)]
                nk_ = _nks[ji % len(_nks)]
                ji += 1
                ticket(None, True, 8 if nk_ == 3 else nk_ + 8, kap, al,
                       drng=_jrng, nk=nk_)
        except Exception:
            pass
        if cands:
            polish(min(cands, key=lambda c: c[0])[1], seed=2077 + soff,
    ...
    if forced:
        if wid == 1:
            ...
            while True:
                ...
                if time.time() >= cap:
                    break
            if cands:
                polish(min(cands, key=lambda c: c[0])[1], seed=555 + soff,
                       alts=pick_alts(cands))
        else:
            ...
            for order_fn, g in ((lambda: _edd_order(blocks_data), 0.5),
                ...
                if time.time() >= cap:
                    break
            if cands:
                polish(min(cands, key=lambda c: c[0])[1], seed=1313 + soff,
                       alts=pick_alts(cands))
    else:
        ...
        if cands and time.time() < deadline:
            polish(min(cands, key=lambda c: c[0])[1], seed=iseed + soff,
                   alts=pick_alts(cands))
...
def _merge_recombine(prob_info, bays, bay_u, w1, w2, w3, pool, incompat_pairs,
    ...
    # this is direct search power on it. Env-overridable for A/B.
    import os as _osm
    import multiprocessing as _mpm
    solver.parameters.num_search_workers = int(_osm.environ.get(
        "OGC_MERGE_WORKERS", str(max(4, _mpm.cpu_count() or 4))))
    try:
        status = solver.Solve(m)
    except Exception:
        return None
...
def _algorithm_portfolio(prob_info, timelimit, t_start):
    ...
    if len(prob_info["blocks"]) >= 250 and _is_forced(prob_info, _pre_bays):
        # jv8: giants get LIGHT extra workers too. W0..W3 unchanged (their
        # RSS budget stands); every wid>=4 maps to the W3-giant repack-
        # specialist slot with soff>0 (see _run_strategy), which skips the
        # heavy nm_elig seed builds -> sparse construct (seconds) + inbox-
        # seeded improve ~1.2GB each. 4w ~4GB measured on 38 + 3 light ->
        # ~8GB, under the 16GB budget. Each replica is an independent
        # repack-polish draw of the global leader (directly attacks 37's
        # measured {5,699,640/5,807,047} bimodality and adds draws on
        # 27/38/39, where every historical win was polish-draw shaped).
        nw = max(min(nw, 4), min(8, _mp.cpu_count() or 1))  # jv9: full width
    else:
        # jv7 CAPACITY SCALING: non-giant instances are light (<1GB/worker,
        # no dense Shapely giants), so the 4-worker cap wastes ~40% of an
        # 8-core machine. Spawn up to 3 extra workers (W4..W6 = seed-
        # diversified W1/W3/W2 replicas, see _run_strategy) leaving one core
