# myalgorithm_30c.py  --  v30c = v30 (30a/30b machinery kept, dead-but-neutral) +
#   BAND-REPAIR DESTROY MODE in _improve() (heuristic_30.md Improvement C),
#   gated STATICALLY to the prob_26 class only.
#   Verdict (30a/30b KILLED, byte-flat {26,37}; on prob_26 the W1 explorer budget
#   is exhausted before the jitter tail so the W1 trio never fired -- append-only
#   W1 delivery is structurally dead there). 30c moves the height-aware lever into
#   the improver, aimed by the fragmentation diagnostic instead of tardiness.
#   GATE (static, no runtime density): forced AND n<=200 AND max bay height<=21
#   AND w1>=6000 AND total_min_area/total_bay_area <= BAND_RATIO_CUT(=3.85). This
#   is True ONLY for prob_26 (ratio 3.624); prob_27 (same n/height/w1 but ratio
#   4.120, saturated) is EXCLUDED -- ejection there is 28b's regression. Giants
#   (38/39 n=250) excluded by n; 31/33 by bay height; 37 by n; prob_1 by forced.
#   MECHANISM (destroy-mode rotation, only when the gate holds AND tardy exist,
#   deterministic cadence every BAND_K=5 rounds, NO rng in the gate/pick):
#     (i)  peak-queue time t* over the incumbent (cached per generation, invalid
#          on accept); (ii) per bay at t*, build the layer-0 occupancy, per-column
#          free-height above the (short) residents, find the widest maximal
#          "shredded" band (free height < median queued bbox height); (iii) the
#          short residents (bbox height < bay_H) whose x-range overlaps the band
#          are the guillotiners; (iv) destroy up to 4 of them (due-slack>=0 first)
#          plus up to 2 tardiest queued-at-t* blocks; (v) repair TALLEST-FIRST
#          with hmatch=True anchor scoring threaded into the raster repair path
#          (_place_block -> _find_earliest_slot_raster -> _order_cells). Accept is
#          obj-gated (min-wins) -- kick/restart are forced False in band rounds.
#   Byte-neutral when the gate is False: the destroy selection and repair order
#   fall through to the exact v30 chain, _place_block/hmatch default False, and
#   the band pick is never called (short-circuited on band_gate) -> no rng moved.
# ---------------------------------------------------------------------------
# myalgorithm_30.py  --  v30 = v25 + HEIGHT-AWARE ANTI-FRAGMENTATION packing
#   (heuristic_30.md rungs 30a + 30b), delivered as APPENDED W1/W2 lottery
#   tickets on the eligible tardy class only. v25 is the SOTA substrate; the diff
#   is minimal and byte-neutral wherever the new tickets do not fire.
#   Motivation (diag_gaps26.py): prob_26 is FRAGMENTATION-bound HEIGHT-wise --
#   burst layer-0 free fraction 0.481 (bays half empty) yet 87.5% of burst
#   (sample,bay) moments have largest-free-rect < EVERY queued bbox; median free
#   rect 23w x 7h vs queued 22w x 14h (width matches, height halves); shallow-
#   strip bays (16-27 tall) get guillotined into thin bands by near-full-height
#   residents; crane-only blocking 0.0%. The lever, never tried in 29 versions,
#   is PLACEMENT-SHAPE discipline that minimizes the fragmentation an entry
#   leaves behind (all prior levers only FOUND anchors; none chose the anchor
#   that leaves the healthiest residual space).
#   30a HEIGHT-UTILIZATION CONTACT SCORING (_order_cells, hmatch flag): on the
#     contact-scoring path, add per-anchor (vectorized, integer, no shapely):
#       + top-alignment bonus (block top edge y+h flush with the bay ceiling or
#         an adjacent resident's top edge within +-1);
#       - height-mismatch penalty (|neighbor top - block h| over left/right
#         contacting residents, normalized);
#       - tall-column-waste penalty (free_height - block_h over the block's
#         columns when it does not reach the ceiling -> shredding a tall column).
#     Single knob HMATCH_W; relative constants HM_ALIGN/HM_MISMATCH/HM_WASTE.
#     hmatch=False (every existing call) leaves the score byte-identical.
#   30b HEIGHT-DESC ADMISSION TIE-BREAK (_dispatch_construct, tallfirst flag):
#     ATC near-ties (within the multiplicative jitter scale eps) ordered by block
#     bbox height DESC. tallfirst=False leaves the ATC order byte-identical.
#   TICKETS: three new tickets -- (i) hmatch, (ii) tallfirst, (iii) both --
#     APPENDED after the ENTIRE v25 W1 rotation (incl. the jitter tail) and after
#     the v25 W2 forced rotation, gated on the existing near_k eligibility
#     (forced & w1>=6000 & overload>0.65 == nm_elig); sub-second scoring, no
#     solver, leftover-budget only -> every existing draw is byte-identical and
#     prob_1 (not forced) never enters these branches.
# ---------------------------------------------------------------------------
...
def _place_block(bi, blk, bays, sched, bay_loads, bay_u, w1, w2, w3,
                 ...
                 util_gamma=0.0, raster=None, hmatch=False):
...
# v30c BAND-REPAIR DESTROY constants (heuristic_30.md Improvement C). Consulted
# only inside _improve() when the static 26-class gate holds (band_gate) -> the
# mode never fires elsewhere and the improver stays byte-identical to v30/v25.
BAND_RATIO_CUT = 3.85    # total_min_area/total_bay_area cut: 26=3.624 in, 27=4.120 out
BAND_K = 5               # deterministic cadence: fire every 5th round
BAND_MAX_GUILL = 4       # max short "guillotiner" residents destroyed per fire
BAND_MAX_TARDY = 2       # max tardiest queued-at-t* blocks added to the destroy set
...
def _improve(prob_info, assignments, bays, bay_u, w1, w2, w3, deadline, forced,
    ...
    # v30c BAND-REPAIR gate (static, 26-class only; no rng, no side effects).
    _band_maxH = max(b.height for b in bays)
    _band_ratio = (sum(_min_area(b) for b in blocks_data)
                   / max(1e-9, sum(b.width * b.height for b in bays)))
    band_gate = (forced and n <= 200 and _band_maxH <= 21 and w1 >= 6000
                 and _band_ratio <= BAND_RATIO_CUT and raster is not None
                 and _HAVE_NUMPY)
    _band_src = [None]      # cache: (incumbent identity, t*) per generation
    _band_tstar = [None]

    def _band_bh(bi):
        b = blocks_data[bi]
        uo = _unique_orients(b)
        oi = min(uo, key=lambda o: _orient_area(b, o))
        bb = _orient_bbox(b, oi)
        return bb[3] - bb[1]

    def _band_pick(src):
        """Deterministic (no rng) band-repair destroy pick. Returns
        (removed_set, tallest-first repair order) or (None, None)."""
        # (i) peak-queue t* over the incumbent, cached per generation.
        if _band_src[0] is not src or _band_tstar[0] is None:
            evq = {}
            for bi, a in src.items():
                r0 = int(blocks_data[bi]["release_time"]); en = int(a["entry_time"])
                if r0 < en:
                    evq[r0] = evq.get(r0, 0) + 1
                    evq[en] = evq.get(en, 0) - 1
            run = 0; peak = -1; tstar = None
            for tt in sorted(evq):
                run += evq[tt]
                if run > peak:
                    peak = run; tstar = tt
            _band_src[0] = src; _band_tstar[0] = tstar
        tstar = _band_tstar[0]
        if tstar is None:
            return None, None
        residents = [bi for bi, a in src.items()
                     if int(a["entry_time"]) <= tstar < int(a["exit_time"])]
        queued = [bi for bi, a in src.items()
                  if int(blocks_data[bi]["release_time"]) <= tstar
                  < int(a["entry_time"])]
        if not residents or not queued:
            return None, None
        qh = sorted(_band_bh(bi) for bi in queued)
        med_qh = qh[len(qh) // 2]
        res_by_bay = {}
        for bi in residents:
            res_by_bay.setdefault(src[bi]["bay_id"], []).append(bi)
        best = None            # (band_width, bay_id, c_lo, c_hi)
        for bay_id, rbis in res_by_bay.items():
            H = raster.H[bay_id]; W = raster.W[bay_id]
            col_top = _np.zeros(W, dtype=_np.int32)
            for bi in rbis:
                a = src[bi]
                mask, cx0, cy0 = raster.mask(bi, a["orient_idx"])
                m0 = mask[0]                       # layer-0 footprint
                MH, MW = m0.shape
                r0 = int(a["y"]) + cy0; c0 = int(a["x"]) + cx0
                for cc in range(MW):
                    col = c0 + cc
                    if col < 0 or col >= W:
                        continue
                    rows = _np.nonzero(m0[:, cc])[0]
                    if len(rows) == 0:
                        continue
                    top = r0 + int(rows.max()) + 1
                    if top > col_top[col]:
                        col_top[col] = top
            band_h = H - col_top
            shred = (col_top > 0) & (band_h > 0) & (band_h < med_qh)
            run = 0; start = 0; bestrun = (0, 0, 0)
            for c in range(W):
                if shred[c]:
                    if run == 0:
                        start = c
                    ...
                    run = 0
            if bestrun[0] > 0 and (best is None or bestrun[0] > best[0]):
                best = (bestrun[0], bay_id, bestrun[1], bestrun[2])
        if best is None:
            return None, None
        _bw, bay_id, c_lo, c_hi = best
        H = raster.H[bay_id]
        guill = []
        for bi in res_by_bay[bay_id]:
            a = src[bi]
            if _band_bh(bi) >= H:                  # full-height -> not a guillotiner
                continue
            mask, cx0, cy0 = raster.mask(bi, a["orient_idx"])
            MW = mask[0].shape[1]
            x0 = int(a["x"]) + cx0; x1 = x0 + MW - 1
            if x1 < c_lo or x0 > c_hi:             # no x-overlap with the band
                continue
            guill.append(bi)
        if not guill:
            return None, None
        # due-slack >= 0 first (they can afford to move), then shortest first
        guill.sort(key=lambda bi: (
            (int(blocks_data[bi]["due_date"]) - int(src[bi]["exit_time"])) < 0,
            _band_bh(bi)))
        chosen = guill[:BAND_MAX_GUILL]
        tq = [bi for bi in queued
              if int(src[bi]["exit_time"]) > int(blocks_data[bi]["due_date"])]
        tq.sort(key=lambda bi: -(int(src[bi]["exit_time"])
                                 - int(blocks_data[bi]["due_date"])))
        chosen = chosen + tq[:BAND_MAX_TARDY]
        removed = set(chosen)
        if not removed:
            return None, None
        rem_order = sorted(removed, key=lambda bi: -_band_bh(bi))  # tallest first
        return removed, rem_order
    ...
    while time.time() < deadline:
        ...
        if (repack_every > 0 and raster is not None and best_tardy > 0
            ...
            if work is not None and len(work) == n:
                ...
                if new_o1 < best_o1 - 1e-9:
                    ...
                    since_o1 += 1
                continue
        # v30c band-repair destroy pick (26-class gate; deterministic cadence,
        # no rng). When it fires, kick/restart are forced off so accept is
        # obj-gated (min-wins) and the rng destroy chain below is skipped.
        band = False
        band_rem_order = None
        if band_gate and best_tardy > 0 and rounds % BAND_K == 0:
            _brm, _bro = _band_pick(cur)
            if _brm:
                removed = _brm; band_rem_order = _bro; band = True
        kick = (not band) and (no_improve >= KICK)
        ...
        restart = ((not band)
                   and since_o1 >= RESTART_AFTER and since_o1 % RESTART_EVERY == 0)
        src = cur
        if band:
            pass                 # removed/rem_order already set by _band_pick
        elif restart:
            n_restart += 1
            src = best_assign
        ...
        if band:
            rem_order = band_rem_order          # v30c: height-matched, tallest first
        elif spread and priority:
        ...
        for bi in rem_order:
            ...
            place = _place_block(bi, blk, bays, sched, bay_loads, bay_u, w1, w2, w3,
                                 forced=forced, slot_time_cap=40, slot_pos_cap=30,
                                 bay_order=bo, raster=use_raster, hmatch=band)
            if place is None:
                place = _force_place(bi, blk, bays, sched)
...
# v30a HEIGHT-UTILIZATION contact-scoring weights (heuristic_30.md). Consulted
# ONLY when _order_cells is called with hmatch=True (the appended anti-frag
# tickets); hmatch defaults False so every other call is byte-identical to v25.
HMATCH_W = 4.0           # single knob scaling the whole fragmentation term
HM_ALIGN = 2.0           #  + per top-alignment (ceiling / neighbor top) match
HM_MISMATCH = 1.5        #  - per unit normalized left/right height mismatch
HM_WASTE = 1.5           #  - per unit normalized tall-column waste


def _order_cells(raster, feas, cx0, cy0, bi, oi, W, occ_fp, prefer_contact, rng,
                 ovh_bay=None, ovh_w=2.0, hmatch=False):
            ...
            if mask.shape[0] > 1:
                ...
                if up.any():
                    ...
                    cval = cval - float(ovh_w) * poison[r, c].astype(
                        _np.float64)
        if hmatch:
            # v30a HEIGHT-UTILIZATION term (vectorized over anchors; integer
            # raster occupancy, no shapely). occ_fp is the current bay footprint;
            # bfp is the block footprint. Coordinates: r=row(=y), block occupies
            # rows [r, r+MHb); "top edge" = r+MHb, "ceiling" = bay height Hh.
            Hh, Wh = occ_fp.shape
            MHb, MWb = bfp.shape
            of = occ_fp.astype(bool)
            rows_idx = _np.arange(Hh, dtype=_np.int32)[:, None]
            col_top = _np.where(of, rows_idx + 1, 0).max(axis=0)   # (Wh,) top ext
            col_free = Hh - of.sum(axis=0).astype(_np.int32)       # (Wh,) free ht
            top_edge = r + MHb                                     # (K,)
            ceil_flush = (_np.abs(top_edge - Hh) <= 1)
            lc = c - 1; rcn = c + MWb
            lc_ok = lc >= 0; rc_ok = rcn < Wh
            lt = _np.where(lc_ok, col_top[_np.clip(lc, 0, Wh - 1)], 0)
            rt = _np.where(rc_ok, col_top[_np.clip(rcn, 0, Wh - 1)], 0)
            l_occ = lc_ok & (lt > 0); r_occ = rc_ok & (rt > 0)
            align = (ceil_flush.astype(_np.float64)
                     + (l_occ & (_np.abs(lt - top_edge) <= 1)).astype(_np.float64)
                     + (r_occ & (_np.abs(rt - top_edge) <= 1)).astype(_np.float64))
            mism = (l_occ * _np.abs(lt - MHb)
                    + r_occ * _np.abs(rt - MHb)).astype(_np.float64)
            wcol = _np.maximum(0, col_free - MHb).astype(_np.float64)   # (Wh,)
            P = _np.concatenate(([0.0], _np.cumsum(wcol)))
            wastewin = (P[_np.clip(c + MWb, 0, Wh)] - P[_np.clip(c, 0, Wh)])
            not_ceil = (~ceil_flush).astype(_np.float64)
            frag = HMATCH_W * (HM_ALIGN * align
                               - HM_MISMATCH * (mism / max(1, Hh))
                               - HM_WASTE * not_ceil
                               * (wastewin / max(1, Hh * MWb)))
            cval = cval + frag
...
def _dispatch_construct(prob_info, bays, bay_u, w1, w2, w3, deadline, raster,
                        ...
                        ovh=False, hmatch=False, tallfirst=False):
    ...
    # v30b: min-area-orient bbox height per block (admission tie-break key);
    # computed only when tallfirst is on so the default path does zero extra work.
    if tallfirst:
        bheights = []
        for b in blocks_data:
            uo = _unique_orients(b)
            oi_h = min(uo, key=lambda o: _orient_area(b, o))
            bb = _orient_bbox(b, oi_h)
            bheights.append(bb[3] - bb[1])
    else:
        bheights = None
    ...
    while heap:
        ...
        if tallfirst:
            # v30b: break ATC near-ties (within the multiplicative jitter scale
            # eps, measured in log space) by block bbox height DESC -- taller
            # blocks claim wall/flush slots before short blocks fill the bands.
            _av = {bi: atc(bi, t) for bi in queue}
            ordered = sorted(
                queue,
                key=lambda bi: (-int(math.floor(math.log(max(_av[bi], 1e-300))
                                                / max(eps, 1e-9))),
                                -bheights[bi], -_av[bi]))
        else:
...
def _run_strategy(wid, prob_info, timelimit, t_start, push, inbox=None):
    ...
    def dispatch(kappa, gamma, drng=None, alpha=0.0, score_pos=True,
        ...
        a = _dispatch_construct(prob_info, bays, bay_u, w1, w2, w3,
                                ...
                                nearmiss=nearmiss, hmatch=hmatch,
                                tallfirst=tallfirst)
        o = iobj(a)
        push(o, a)
        return o, a
    ...
    if wid == 2 and raster is not None:
        ...
        if forced:
            ...
            for al, ka, ts_, bm in plan:
                ...
                if time.time() >= cap:
                    break
            # v30a/30b: append the height-aware trio AFTER the whole v25 W2
            # forced rotation (every existing draw byte-identical), gated on the
            # near_k eligibility (nm_elig == forced & w1>=6000 & overload>0.65).
            # Sub-second scoring, no solver, leftover budget only.
            if nm_elig:
                for _hm, _tf in ((True, False), (False, True), (True, True)):
                    if time.time() >= cap:
                        break
                    try:
                        cands.append(dispatch(0.5, 0.5, alpha=0.5, beam=True,
                                              dl=cap, nearmiss=8,
                                              hmatch=_hm, tallfirst=_tf))
                    except Exception:
                        break
        else:
            try:
    ...
    if (wid == 1 and raster is not None and forced and w1 >= 6000
        ...
        try:
            ...
            def ticket(tg, bm, nm, kap, al, drng=None, mpc=False,
                       ovh=False, nk=3, hmatch=False, tallfirst=False):
                raster.reset()
                raster.near_k = nk
                a_ = _dispatch_construct(
                    ...
                    nearmiss=nm, mpc=mpc, ovh=ovh, hmatch=hmatch,
                    tallfirst=tallfirst)
                o_ = iobj(a_)
                push(o_, a_)
                cands.append((o_, a_))
            ...
            # v30a/30b: height-aware anti-fragmentation trio, APPENDED after the
            # ENTIRE v25 W1 rotation (incl. the jitter tail) -> every existing
            # draw is byte-identical; each is sub-second scoring (no solver) and
            # runs only in leftover budget before tick_dl. kappa/alpha = the hot
            # 0.5/0.5 head config; nk=3 (hmatch, not near_k, is the lever here).
            for _hm, _tf in ((True, False), (False, True), (True, True)):
                if time.time() >= tick_dl:
                    break
                ticket(None, True, 8, 0.5, 0.5, hmatch=_hm, tallfirst=_tf)
        except Exception:
            pass
        if cands:
