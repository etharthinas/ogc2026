# myalgorithm_jv11.py  --  jv11 = jv9 + W0-RECLAIM PIPELINE REPLICA (giants).
# The 9th worker (wid 8, oversubscribed onto the idle parent core) reruns the
# leader-PRODUCING W0-reclaim pipeline (nk-nestle build -> deep improve ->
# whole_bay -> z3) under a fresh polish trajectory -- attacks the giant
# leader's OWN basin instead of repacking it (jv10 lesson: ADD draws, never
# displace streams). All jv9 streams intact. See heuristics/heuristic_jv11.md.
# ---------------------------------------------------------------------------
...
def _run_strategy(wid, prob_info, timelimit, t_start, push, inbox=None):
    ...
    if wid >= 4:
        ...
        if forced and len(blocks_data) >= 250:
            ...
            # jv11 W0-RECLAIM LIGHT REPLICA: repurpose the wid-6 slot (jv9's
            # DEAF stream, unproven on giants) into a LIGHT rerun of the
            # W0-RECLAIM POLISH chain (improve->whole_bay->z3) under a fresh
            # trajectory (seeds 1616/1617+soff), inbox-seeded from the leader.
            # Every prior giant replica cloned the W3-REPACK chain (a repack
            # of the leader); this attacks the leader's OWN layout with the
            # DEEPER reclaim polish chain. Constant worker count (nw stays 8 =
            # jv9's RSS-proven config -- oversubscribing to 9 workers PAGED
            # prob_38 to a 3.7B empty-bay blowout, canary-confirmed twice).
            # Net vs jv9 = lose the unproven deaf stream, gain the reclaim
            # polish draw; the {27,38,39} A/B measures exactly this.
            if wid == 6:
                wid = 0
            else:
                wid = 3
    ...
    if wid == 0:
        ...
        if reclaim and raster is not None:
            a0 = None
            # jv11 LIGHT W0-RECLAIM REPLICA (soff>0): the heavy nk-nestle +
            # mpc + drain builds are a SECOND full copy of W0's pipeline --
            # canary on prob_38 @9w thrashed RAM, stalled all workers, blew
            # the deadline (412s/180s, empty-bay 3.7B). So the replica SKIPS
            # the rebuilds: it seeds a0 from the inbox LEADER and runs only
            # the reclaim POLISH chain (improve->whole_bay->z3) under a fresh
            # trajectory (seeds 1616/1617+soff) -- same new-basin attack on
            # the leader's own layout, ~repack-worker RSS (no build spike).
            # Falls through to one cheap sparse build if the inbox is empty.
            if soff > 0:
                if inbox is not None:
                    try:
                        while True:
                            _oi, _ai = inbox.get(timeout=8.0)
                            if a0 is None or _oi < _o0:
                                _o0 = _oi
                                a0 = {k: dict(v) for k, v in _ai.items()}
                    except Exception:
                        pass
                if a0 is None:
                    try:
                        a0 = _construct(prob_info, _edd_order(blocks_data),
                                        bays, bay_u, w1, w2, w3,
                                        t_start, t_start, forced=True)
                        _o0 = iobj(a0)
                    except Exception:
                        a0 = None
            try:
                if a0 is not None and soff > 0:
                    pass  # light replica: a0 already seeded, go to polish
                elif nm_elig:
            ...
            if a0 is not None:
                ...
                o1, r1 = improve(a0, seed=1616 + soff, until=wb_at, repack_every=3,
                                 xbay=False)
                base, ob = r1, o1
                ...
                if time.time() < z3_at - 2.0:
                    o2, r2 = improve(base, seed=1617 + soff, until=z3_at,
...
def _algorithm_portfolio(prob_info, timelimit, t_start):
    ...
    if len(prob_info["blocks"]) >= 250 and _is_forced(prob_info, _pre_bays):
        ...
        # jv11: giants stay at nw=8 (jv9's RSS-proven config). Oversubscribing
        # to 9 workers PAGED prob_38 to a 3.7B empty-bay blowout at 390-412s
        # (canary-confirmed twice, heavy AND light replica) -- 8 workers +
        # parent already = 9 procs on 8 cores, and the 36.76M giant saturates
        # RAM. The W0-reclaim replica is delivered by REPURPOSING wid 6 (see
        # _run_strategy), not by adding a worker. OGC_GIANT_NW kept for probes.
        _cap8 = int(_os0.environ.get("OGC_GIANT_NW",
                                     str(min(8, _mp.cpu_count() or 1))))
        nw = max(min(nw, 4), _cap8)
    else:
