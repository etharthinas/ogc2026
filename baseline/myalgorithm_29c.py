    ...
    if wid == 2 and raster is not None:
        ...
        if forced:
            ...
            for al, ka, ts_, bm in plan:
                ...
                if time.time() >= cap:
                    break
            # v29c Improvement C: APPEND ONE fluid-order order-forced ticket AFTER
            # every existing draw (append-only law -- prepending would displace
            # the banked lottery rotation). Only on the reclaim class ({38,27,...}
            # = the structurally overloaded/giant tardy pair the relax@0.7
            # certificate targets); min-wins gating protects the floor.
            if ((len(blocks_data) >= 250 or overload > 1.05)
                    and time.time() < cap):
                try:
                    S_fluid, f_obj1 = _fluid_order(prob_info, bays)
                    res = dispatch(1.0, 0.5, alpha=1.0, nearmiss=8,
                                   order=S_fluid, dl=cap)
                    import os as _os3
