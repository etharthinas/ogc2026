...
# FORCED instances ONLY, harvesting banked obj2/obj3 mass that every historical
# lever ignored (they all chased obj1). v34 tried single greedy relocations here
# and found ZERO feasible moves on space-saturated rocks (prob_31/38: 0 accepts)
# -- but SWAPS / CHAINS (A into B's simultaneously-vacated slot) can still be
# feasible. Offline ceiling (exp_joint_frozen.py): prob_31 = 9 swaps, -4,629
# weighted, official-feasible; prob_38 = 0. Small but real and never-negative.
#   * SOUNDNESS: with all entry/exit FROZEN to the champion's, replay order and
#     co-resident sets are FIXED constants -- only positions vary, so pairwise
#     position-compatibility is EXACT. We reuse the S4 merge's frozen-time
#     predicate verbatim: _merge_incompatible == the exact static reduction of
#     _can_place with both windows fixed. A joint CP-SAT over time-frozen
#     position choices is thus sound AND complete for this move class. obj1 is
#     invariant by construction (entry/exit are never touched).
...
#   * MODEL (`_tail35_cpsat`), SHRUNK for the ~35-50s forced tail (offline model
#     was 169 movable x 40 cands x ~975k constraints -> minutes): MOVABLE =
#     off-preference blocks + Z2-max-bay (max normalized load) blocks ONLY, no
#     spatial expansion, cap 60 by penalty desc; <=10 candidates/block (champion
#     as index 0, then preferred-bay raster anchors from scan_scoped vs FIXED
#     blocks only, _can_place-gated vs the fixed co-residents); pairwise
#     mutual-exclusion constraints only for TIME-OVERLAPPING movable pairs whose
#     candidates SHARE a bay (cross-bay pairs never collide). Objective =
#     w3*obj3 + w2*(linearized Z2); AddHint = champion. Champion being candidate
#     0 for every block means the model always contains the identity solution ->
#     the CP-SAT optimum is never internally worse than the champion.
#   * TIME GUARDS: build+solve share deadline = remaining - 3s safety. The BUILD
#     (candidate loop + pair loop) must not exceed HALF the remaining budget --
#     checked at build_stop = start + (deadline-start)/2 inside both loops; on
#     breach it aborts and returns the champion. The solver gets whatever time
#     remains up to the deadline.
#   * SAFETY (v34 guards kept): (1) champion-is-candidate-0 -> never internally
#     worse; (2) obj1 recomputed via the OFFICIAL check and the tail is rejected
#     unless official obj1 is byte-equal to the champion's (asserts the freeze);
#     (3) the whole solved assignment is rebuilt to operations and re-run through
#     the official utils.check_feasibility -- the crane subtlety (a relocation
#     perturbs BOTH bays' replay, caught only at official stage 5) reverts to the
#     champion on any infeasibility. Returns the better of {tail, champion} by
#     OFFICIAL objective; a failed/empty tail costs nothing. OGC_DEBUG emits:
#     [tail35] forced=True movable=<m> cands_avg=<c> swaps=<s> wdelta=<d> rem_start=<s>.
# =============================================================================
            ...
            if res["feasible"]:
                ...
                if rem_start >= 10.0:
                    try:
                        ...
                        pass
                # v37s CALL SITE (single addition; race path + existing
                # tails above are token-identical to v36). String-embedded
                # order-aware presence-window CP-SAT tail, lazy-loaded via
                # _t37_load() so spawned workers only lex one string literal
                # (near-zero worker-visible code delta vs v36). Same gate /
                # behavior as myalgorithm_37.py: elapsed-guarded (>=120s),
                # official-checker gated, min-wins.
                try:
                    _f37 = _t37_load()
                    if _f37 is not None:
                        t37_sol = _f37(prob_info, assign, bays, bay_u,
...
# =============================================================================
# v37s STRING-EMBEDDED ORDER TAIL. The entire post-race tail (all _t37_*/
# _T37* defs incl. the lazy _t37_cp() ortools getter) is stored as ONE raw
# string literal. Spawned portfolio workers re-import this module but only
# LEX the literal (no AST build / no bytecode compile for its ~700 lines),
# so the worker-visible code delta vs myalgorithm_36.py is near-zero and the
# forced race lotteries are not displaced. _t37_load() exec's the source
# into module globals ONCE, on the first (post-race, parent-side) call, and
# returns the _tail37_order callable. exec into globals() resolves every
# reference (_mkblock, _can_place, _objective, check_feasibility, _Raster,
# _HAVE_NUMPY, time, re, ...) against this module's globals -- all present
# because this file is a byte-copy of myalgorithm_36.py.
# =============================================================================
_T37_SRC = r'''
...
_T37_NS_LOADED = False


def _t37_load():
    global _T37_NS_LOADED
    if not _T37_NS_LOADED:
        exec(compile(_T37_SRC, "<tail37>", "exec"), globals())
        _T37_NS_LOADED = True
    return globals().get("_tail37_order")
