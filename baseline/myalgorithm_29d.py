# myalgorithm_29d.py  --  v29d = v29c with ONE change: the appended W2 giant order-
#   forced ticket derives its forced order from the in-repo CP-SAT relaxation
#   _plan_targets (the source of the relax@0.7 obj1 certificate 1219/615) instead
#   of the greedy ATC fluid loader _fluid_order.
#   Why: 29c spot (prob_38) telemetry -- fluid_obj1=4439 (WORSE than the banked
#   realized obj1 2569, nowhere near the 1219 certificate), realized_obj=48.0M,
#   mean_rank_delta=30.8. The greedy loader's own relaxation was a bad order, so
#   min-wins discarded it (38 byte-flat). The certificate hypothesis (a globally
#   better admission SEQUENCE exists) can only be tested with the CP-SAT order.
#   Mechanism: at the SAME append-only W2 ticket site / same reclaim gate / same
#   dispatch args (order=S, nearmiss=8, dl=cap), call _plan_targets(cap_frac=0.7)
#   once with a short budget (a few seconds, bounded by the room left to `cap`,
#   reusing the v20 caller's floor-a-few-seconds pattern); S = block ids sorted by
#   (target_start, due, id); if the solve returns None (fail/timeout) SKIP the
#   ticket -- no greedy fallback. Telemetry line retagged [OGC v29d plan-order]
#   with plan_obj1 = Sum max(0, target_start+proc-due) over blocks (the relaxation's
#   own tardiness; _plan_targets exposes only the times). _fluid_order stays
#   defined (unused) for provenance. Everything else byte-identical to 29c.
# ---------------------------------------------------------------------------
# myalgorithm_29c.py  --  v29c = v29a substrate (order-forced machinery + W3 replay
#   stream UNCHANGED) + FLUID-ORDER TICKET in the W2 giant lottery
#   (heuristic_29.md Improvement C).
#   Verdict (29a spot): the order-forced replay works (prob_27 reached obj1=1725
#   vs banked 1726, losing only on obj2/obj3) but LOCAL perturbations only move
#   single tardiness units -- we need a GLOBALLY different order. The relax@0.7
#   experiment (ledger_27_diag: obj1 ~1219/615) is a certificate that a much
#   better admission SEQUENCE exists. So v29c feeds that sequence in directly:
#   1. _fluid_order (new module helper): a fast aggregate-area fluid relaxation
#      (REUSING _fluid_targets' capacity model -- capacity = 0.7 * total layer-0
#      bay area, a block occupies its min-orient footprint area for its
#      processing time, no geometry/crane/bay identity), event-driven greedy
#      loader admitting in ATC urgency with due-date tie-break; returns the
#      ADMISSION SEQUENCE (block ids in fluid entry order) + the relaxation's own
#      obj1. < 1s over ~250 blocks. Distinct from _fluid_targets' spt/band/cut
#      SACRIFICE disciplines (which gate admission TIMES and lost in v22/23):
#      this forces only relative ORDER and still admits greedily-early (the A
#      mechanism), realized under true geometry by the order-forced dispatch.
#   2. In the W2 giant/overloaded lottery (wid==2, forced, n>=250 OR overload>1.05
#      -- the reclaim class = {38,27,...}, matching the {38,27} spot), APPEND one
#      order-forced dispatch(order=S_fluid) AFTER every existing ticket
#      (append-only law -- prepending would displace the banked lottery draws).
#      min-wins gating protects the floor; every preceding draw is byte-identical.
#   3. OGC_DEBUG telemetry: one line per giant with the fluid obj1, the realized
#      objective of the fluid-order candidate, and the mean |rank delta| between
#      the fluid order and the realized champion order (the ledger's requested
#      realized-vs-fluid order distance).
#   order= machinery, the W3 replay stream, and W1/W0 are byte-exact v29a; the
#   only behavioural change is the one appended W2 ticket on the reclaim class.
# ---------------------------------------------------------------------------
# myalgorithm_29a.py  --  v29a = v28 substrate + ORDER-FORCED replay search in the
#   W3 giant specialist's first-seed slot (heuristic_29.md Improvement A).
#   Verdict (heuristic_28.md Results): the giant improver is round-starved (38:
#   3-17 rounds, since_o1 <= 16), improver-level escapes can NEVER touch 38 and
#   round-stealing regresses (28b +1.9M); the W3 first-seed slot is free-but-
#   worthless (26b + 28c byte-flat twice). So v29a moves the sequencing search
#   out of the improver and into the ADMISSION ORDER, and spends the worthless
#   first-seed slot on it.
#   TWO PIECES:
#   1. ORDER-FORCED dispatch mode -- new `order=None` param on _dispatch_construct
#      (threaded through the `dispatch` closure exactly like nothing else touches
#      it). When order=S' (a list of block ids), each event admits the earliest-
#      in-S' queued block first (full scan + near-miss), with bounded skip-ahead
#      of ORDER_LOOKAHEAD=3 consecutive unplaceable blocks (skipped blocks retry
#      first at later events), and a hard ATC escape when the forced head stalls
#      for ORDER_STALL=8 consecutive events. order=None is BYTE-IDENTICAL to v28
#      (the new branch and its pre-loop state are both gated on order is not None).
#   2. REPLAY-SEARCH STREAM replacing the near_k=32 first seed (nm_elig giants):
#      take the current inbox champion (or one plain beam dispatch if none yet),
#      extract its realized admission sequence S = ids sorted by (entry_time, id),
#      then loop to the slot's 0.22*window deadline perturbing S -> S' (tardiness-
#      weighted pull-earlier past slack-rich predecessors; every 3rd move a
#      block-of-8 reversal around a tardy-cluster peak) and order-forced-
#      dispatching S'; keep-if-better vs the slot's local best. Every candidate is
#      pushed through the existing obj-gated push, and the local best raw feeds the
#      UNCHANGED second (ovh) seed + improve. Dedicated random.Random(9291) so no
#      other rng stream is displaced (order-forced dispatch draws no rng: atc
#      jitter rng is None here). W1/W0/W2 and every non-giant path are byte-exact.
# ---------------------------------------------------------------------------
# myalgorithm_28.py  --  v28a = v27a substrate (x3.56 scans, byte-neutral vs
#   v25) + RE-ENABLED deep partial restart + cross-bay rebalance in _improve().
#   Both were disabled since v9 (RESTART_AFTER = REBAL_AFTER = 10**9). Verdicts
#   (heuristic_27 Results + ledger_27_diag): the eligible-8 reproduce v25
#   byte-for-byte under x3.56 scans -> every stream CONVERGES within 600s then
#   spins on wasted rounds; sequencing (which blocks hold the saturated bays),
#   not admission search, is the remaining lever. So spend the wasted
#   post-convergence rounds on basin-hopping:
#     - RESTART_AFTER = 60, RESTART_EVERY = 25 (fires at since_o1 = 75,100,...),
#       RESTART_FRAC = 0.45; gate is since_o1 (obj1-stall) so descent rounds are
#       never stolen (round-starved giants never reach the gate).
#     - REBAL_AFTER = 80 (cross-bay rebalance on truly-converged instances).
#     - NEW restart rebuild-order variant: every 2nd restart fire reinserts the
#       removed set in volume-normalized-ATC order (due_date asc, then
#       footprint-area x processing asc; no rng consumed) instead of the
#       shuffled _repair_order -- the tardiness-sequencing bias the diagnostic
#       calls for, ungated.
#   Safety by construction: pre-gate rng stream untouched (restart branch
#   consumes rng only when it fires) -> pre-convergence byte-identical to v27a;
#   `best_assign` monotone -> returned solution can only improve or tie. Also
#   fixed the never-incremented n_rebal debug counter (OGC_DEBUG).
# ---------------------------------------------------------------------------
# myalgorithm_26.py  --  v26 = v25 + ONE surviving (measured-neutral) change:
#   26b: the W3 giant specialist's FIRST seed build -> DEEP-NESTLE near_k=32 /
#        nearmiss=40 (ovh second seed byte-exact). Spot {38,39}: byte-flat --
#        the min-wins improve never beat the inbox leader; kept as floor-safe.
# FAILED AND REVERTED (see heuristic_26.md Results):
#   26a: near_k=32 build1 in the W0-reclaimed deep pipeline (38 +1.89M).
#   26c: appended W1 mpc x nk32 + nk{48,64} tickets (38 +1.89M, byte-identical
#        fallback to 26a's -- 38's winner is a W1<->W0-reclaimed cross-worker
#        coupling; both sides FROZEN for 38).
# Outputs are byte-identical to v25 on every measured cell (38/39/27 spots +
# prob_1 protect). W0-reclaimed and the whole W1 rotation are byte-exact v25.
#
...
# -----------------------------------------------------------------------------
# v27 SCAN-THROUGHPUT KERNEL (heuristic_27 Improvement A).
# The raster hot loop was a dense O(R*C*MH*MW) einsum cross-correlation per
# layer, per (block,orient,bay,ver) -- ~92% of improver wall-clock (ledger_19).
# Replace it with a numba SATURATING correlation: for each anchor accumulate the
# mask/occupancy overlap count but stop once it reaches near_cap = near_k+1.
# feas = (count == 0) and near = (0 < count <= near_k) are then BYTE-IDENTICAL
# to the einsum path (capping never changes either verdict), while dense giants
# -- where almost every anchor is infeasible -- early-exit after a few cells.
# Measured (session microbench, real prob_38/27 dims, 0/400 exactness
# mismatches): x15-23 at near_k=3 (the default), x1.76-2.63 at near_k=32.
# numba is submission-legal (ogc2026_env.yml). Guarded so the eval cannot die
# on a missing import: numba -> (FFT hybrid) -> einsum, all exact.
try:
...
if _HAVE_NUMBA:
    @_njit(cache=False, fastmath=False, boundscheck=False)
    def _corr_sat(union_stack, nlayers_eff, mc_layer, mc_r, mc_c, R, C, near_cap):
        """total[r,c] = min(near_cap, # mask cells that overlap an occupied
        union cell at anchor (r,c)), summed over layers < nlayers_eff. Exact for
        feas (==0) and near (0<t<=near_cap-1); saturates above via per-anchor
        early-exit. mc_* are the nonzero mask-cell coords (all layers)."""
        total = _np.zeros((R, C), dtype=_np.int32)
        ncells = mc_layer.shape[0]
        for r in range(R):
            for c in range(C):
                cnt = 0
                for j in range(ncells):
                    ...
                    if L >= nlayers_eff:
                        continue
                    if union_stack[L, r + mc_r[j], c + mc_c[j]]:
                        cnt += 1
                        if cnt >= near_cap:
                            break
                total[r, c] = cnt
...
def _corr_counts(unions, mask, mc, R, C, near_cap):
    """Overlap-count grid (R,C) int32, saturated at near_cap. `unions` is the
    per-layer occupancy union list (union_ge[k], bool (H,W)); `mask` is the
    (nl,MH,MW) block mask; `mc` is the cached (layer,r,c) nonzero mask-cell
    arrays (or None -> derive). Iterates k over min(nl, len(unions)) layers,
    exactly matching the einsum path's `if k > maxL: break`."""
    nl = mask.shape[0]
    nle = len(unions)
    if nle > nl:
        nle = nl
    if nle <= 0:
        return _np.zeros((R, C), dtype=_np.int32)
    if _HAVE_NUMBA:
        H, W = unions[0].shape
        stack = _np.empty((nle, H, W), dtype=_np.uint8)
        for k in range(nle):
            stack[k] = unions[k]
        if mc is None:
            nz = _np.argwhere(mask)
            mcl = nz[:, 0].astype(_np.int32)
            mcr = nz[:, 1].astype(_np.int32)
            mcc = nz[:, 2].astype(_np.int32)
        else:
            mcl, mcr, mcc = mc
        return _corr_sat(stack, nle, mcl, mcr, mcc, R, C, int(near_cap))
    # ---- fallback: exact einsum (uncapped; caller derives feas/near) ----
...
def _warmup_kernel():
    """Compile the numba scan kernel at import so the first real scan is fast
    (per spawned worker; cache=False -- eval pycache writability unknown).
    Budget ~1-3s; any failure silently leaves the einsum fallback in place."""
    if not (_HAVE_NUMBA and _HAVE_NUMPY):
        return
    try:
        u = [_np.zeros((4, 4), dtype=bool)]
        m = _np.ones((1, 2, 2), dtype=_np.uint8)
        _corr_counts(u, m, None, 3, 3, 4)
    except Exception:  # pragma: no cover
        pass
...
def _improve(prob_info, assignments, bays, bay_u, w1, w2, w3, deadline, forced,
    ...
    KICK = 6
    REBAL_AFTER = 80      # v28a: cross-bay rebalance ON for converged instances
    RESTART_AFTER = 60    # v28a: deep partial-restart ON (fires at since_o1=75,100..)
...
class _Raster:
    ...
    *touches* the closed unit square of local cell (cx0+j, cy0+i). Because the
    ...
    def __init__(self, prob_info, bays):
        ...
        self._mcells = {}                     # v27: (bi,oi)->(layer,r,c) int32
        #                    nonzero mask-cell coords for the numba scan kernel
    ...
    def _mask_cells(self, bi, oi):
        """v27: cached (layer,r,c) int32 coord arrays of the block mask's set
        cells (all layers), for the numba saturating-correlation scan kernel."""
        key = (bi, oi)
        mc = self._mcells.get(key)
        if mc is not None:
            return mc
        mask = self.mask(bi, oi)[0]
        nz = _np.argwhere(mask)
        mc = (nz[:, 0].astype(_np.int32).copy(),
              nz[:, 1].astype(_np.int32).copy(),
              nz[:, 2].astype(_np.int32).copy())
        self._mcells[key] = mc
        return mc
    ...
    def scan(self, bay, bi, oi):
        ...
        # v27: numba saturating correlation (feas/near byte-identical to einsum).
        near_cap = self.near_k + 1 if self.near_k >= 1 else 1
        total = _corr_counts(unions, mask, self._mask_cells(bi, oi),
                             R, C, near_cap)
        feas = (total == 0)
    ...
    def scan_scoped(self, bay, actives, bi, oi, want_near=False):
        ...
        # v27: numba saturating correlation (feas/near byte-identical to einsum).
        near_cap = self.near_k + 1 if self.near_k >= 1 else 1
        total = _corr_counts(union_ge, mask, self._mask_cells(bi, oi),
                             R, C, near_cap)
        if want_near:
...
# v29c FLUID-ORDER relaxation (heuristic_29.md Improvement C). Aggregate-area
# fluid/bin-capacity relaxation reusing _fluid_targets' capacity model.
FLUID_CEFF = 0.7         # capacity = FLUID_CEFF * total layer-0 bay area
PLAN_CAP_FRAC = 0.7      # v29d: cap_frac for _plan_targets (the relax@0.7 certificate)


def _fluid_order(prob_info, bays, c_eff=FLUID_CEFF):
    """v29c: fluid/bin-capacity relaxation ORDER for a giant instance.

    Bays collapse to a single capacity pool = c_eff * total layer-0 bay area
    (width*height is the layer-0 footprint); each block is an (area, proc,
    release, due) demand that occupies its MIN-ORIENT footprint area for `proc`
    time once admitted -- no geometry, no crane, no bay identity (the same
    aggregate-area relaxation _fluid_targets uses). Event-driven greedy loader:
    at each event (releases + admitted-block exits) free finished blocks, then
    admit released-and-waiting blocks in ATC urgency order (due-date tie-break)
    whose area fits the remaining capacity; a block whose area alone exceeds the
    pool is force-admitted when the pool is empty so the loader always advances.
    Returns (order, obj1): `order` = block ids in fluid admission (entry) order,
    `obj1` = the relaxation's own sum of tardiness. Deterministic, < 1s over
    ~250 blocks. Distinct from _fluid_targets (spt/band/cut SACRIFICE gates on
    admission TIMES): this yields a globally different relative ORDER, realized
    greedily-early by the order-forced dispatch (mechanism A)."""
    import heapq
    blocks = prob_info["blocks"]
    n = len(blocks)
    if n == 0:
    ...
    total_area = sum(b.width * b.height for b in bays)
    cap = c_eff * total_area
    areas = [float(_min_area(b)) for b in blocks]
    procs = [max(1, int(b["processing_time"])) for b in blocks]
    rels = [int(b["release_time"]) for b in blocks]
    dues = [int(b["due_date"]) for b in blocks]
    pbar = max(1.0, sum(procs) / max(1, n))

    def atc_key(bi, t):
        # ATC urgency (kappa=1, no volume triage); higher = more urgent.
        slack = dues[bi] - procs[bi] - t
        return (1.0 / procs[bi]) * math.exp(-max(0.0, slack) / pbar)

    rel_sorted = sorted(range(n), key=lambda i: (rels[i], dues[i], i))
    rp = 0
    in_use = 0.0
    exits = []                      # heap of (exit_time, area)
    ev = list(set(rels))            # event times seeded with all releases
    heapq.heapify(ev)
    waiting = set()
    order = []
    entry_time = {}
    last_t = rels[rel_sorted[0]]
    while len(order) < n:
        if ev:
            t = heapq.heappop(ev)
        elif exits:
            ...
            t = last_t
        last_t = t
        # free finished blocks
        while exits and exits[0][0] <= t:
            _, a = heapq.heappop(exits)
            in_use -= a
        # release newly available blocks
        while rp < n and rels[rel_sorted[rp]] <= t:
            waiting.add(rel_sorted[rp]); rp += 1
        if not waiting:
            if rp < n:                       # jump to next release
                continue
            if exits:                        # or next exit frees space
                continue
            break
        # admit in ATC order while area fits (due-date tie-break via -atc,due,id)
        cand = sorted(waiting, key=lambda bi: (-atc_key(bi, t), dues[bi], bi))
        admitted_any = False
        for bi in cand:
            if in_use + areas[bi] <= cap + 1e-9:
                in_use += areas[bi]
                heapq.heappush(exits, (t + procs[bi], areas[bi]))
                order.append(bi); entry_time[bi] = t
                waiting.discard(bi)
                admitted_any = True
        if not admitted_any:
            if in_use <= 1e-9:
                # pool empty but head still won't fit (area > cap) -> force it
                bi = cand[0]
                in_use += areas[bi]
                heapq.heappush(exits, (t + procs[bi], areas[bi]))
                order.append(bi); entry_time[bi] = t
                waiting.discard(bi)
            elif not ev and not exits:
                break                        # no way to make progress
    # any leftover (defensive): append in release order
    for bi in rel_sorted:
        if bi not in entry_time:
            order.append(bi); entry_time[bi] = rels[bi]
    obj1 = sum(max(0, entry_time[bi] + procs[bi] - dues[bi]) for bi in range(n))
    return order, float(obj1)


# v29a ORDER-FORCED dispatch constants (heuristic_29.md Improvement A). Consulted
# ONLY when _dispatch_construct is called with order != None (the W3-giant replay
# seed); every other call passes order=None -> byte-identical to v28.
ORDER_LOOKAHEAD = 3      # k: max consecutive unplaceable skips before ending event
ORDER_STALL = 8          # consecutive events the forced head may stall -> ATC escape


def _replay_perturb(order, exit_of, blocks_data, rng, count):
    """v29a: perturb a realized admission order (list of block ids) into a
    neighbour. Two moves, chosen by `count`:
      * every 3rd call (or when nothing is tardy): reverse a contiguous window of
        8 positions centred on a burst peak (a tardy block's position proxy).
      * otherwise: pick a tardy block tardiness-weighted and move it EARLIER past
        the contiguous run of immediate predecessors that have due-slack >= 0
        (exit <= due), to a randomly sampled insertion point in that run -- the
        resident<->queued 2-exchange at the ORDER level. Falls back to an adjacent
        swap if the block has no slack-rich predecessor. `exit_of` maps block id
        -> realized exit_time of the current local-best assignment (the slack /
        tardiness certificate); rng is a dedicated stream so no other is moved.
    Returns a NEW list; never mutates `order`."""
    m = len(order)
    if m < 4:
        return list(order)

    def tard(bi):
        return max(0, exit_of.get(bi, 0) - int(blocks_data[bi]["due_date"]))

    tardy = [bi for bi in order if tard(bi) > 0]
    if count % 3 == 2 or not tardy:
        c = order.index(rng.choice(tardy)) if tardy else rng.randrange(m)
        hi = min(m, max(0, c - 4) + 8)
        lo = max(0, hi - 8)
        out = list(order)
        out[lo:hi] = out[lo:hi][::-1]
        return out
    # tardiness-weighted pick of the block to pull earlier
    wsum = float(sum(tard(bi) for bi in tardy))
    r = rng.uniform(0.0, wsum)
    acc = 0.0
    i = tardy[-1]
    for bi in tardy:
        acc += tard(bi)
    ...
    while j >= 0:
        pj = order[j]
        if exit_of.get(pj, 0) - int(blocks_data[pj]["due_date"]) <= 0:  # slack>=0
            lb = j
            j -= 1
        else:
            break
    out = list(order)
    if lb >= cur:                      # no slack-rich predecessor -> adjacent swap
        if cur > 0:
            out[cur - 1], out[cur] = out[cur], out[cur - 1]
        return out
    ins = rng.randrange(lb, cur)
    out.pop(cur)
    out.insert(ins, i)
    return out
...
def _dispatch_construct(prob_info, bays, bay_u, w1, w2, w3, deadline, raster,
    ...
    placed_cnt = 0
    last_t = None
    # v29a ORDER-FORCED state (all inert unless order is not None -> byte-exact v28)
    if order is not None:
        _order_rank = {bi: i for i, bi in enumerate(order)}
        for bi in range(n):                     # blocks missing from S admit last
            if bi not in _order_rank:
                _order_rank[bi] = len(order) + bi
    else:
        _order_rank = None
    _stall_head = None
    _stall_cnt = 0
    ...
    while heap:
        ...
        if order is not None:
            # v29a ORDER-FORCED admission (heuristic_29.md A): admit the earliest-
            # in-`order` queued block first, then the next, etc. Bounded skip-
            # ahead -- after ORDER_LOOKAHEAD consecutive unplaceable blocks stop
            # the event (the rest wait and, being earlier in `order`, are retried
            # first next event). Hard stall escape: if the forced head has failed
            # to place for ORDER_STALL consecutive events, admit THIS event in ATC
            # order to break the deadlock. try_place/near-miss/scoring unchanged.
            q_by_order = sorted(queue, key=lambda bi: _order_rank[bi])
            head = None
            for bi in q_by_order:
                if targets is not None and t < targets[bi]:
                    continue
                head = bi
                break
            if head != _stall_head:
                _stall_head = head
                _stall_cnt = 0
            if _stall_cnt >= ORDER_STALL:
                for bi in ordered:                       # ATC escape
                    if time.time() > deadline:
                        break
                    if targets is not None and t < targets[bi]:
                        continue
                ...
                for bi in q_by_order:
                    ...
                    if place is not None:
                        ...
                        skipped_run += 1
                        if skipped_run > ORDER_LOOKAHEAD:
                            break
                if head is not None and head in queue:
                    _stall_cnt += 1
                else:
                    _stall_cnt = 0
                    _stall_head = None
        elif beam and len(ordered) >= beam_m:
...
def _run_strategy(wid, prob_info, timelimit, t_start, push, inbox=None):
    ...
    def dispatch(kappa, gamma, drng=None, alpha=0.0, score_pos=True,
                 tspec=None, beam=False, dl=None, nearmiss=0, order=None):
        raster.reset()
        ...
        if reclaim and raster is not None:
            ...
            try:
                if nm_elig:
                    ...
                    # v26a (FAILED, reverted): a near_k=32 build1 here regressed
                    # 38 by +1.89M -- swapping a winning deep pipeline's build
                    # input displaced its own polish basin (v24 law). W0-reclaimed
                    # build1 is byte-exact v25.
    ...
    if wid == 2 and raster is not None:
        ...
        if forced:
            ...
            for al, ka, ts_, bm in plan:
                ...
                if time.time() >= cap:
                    break
            # v29d Improvement C (V2): APPEND ONE order-forced ticket whose order
            # is derived from the in-repo CP-SAT relaxation _plan_targets -- the
            # source of the relax@0.7 obj1 certificate (1219/615) -- instead of the
            # greedy ATC fluid loader (29c's _fluid_order gave a WORSE relaxation
            # tardiness than the banked realized obj1, so the forced order was bad
            # and min-wins discarded it). S = block ids sorted by CP-SAT target
            # start (due-date, id tie-break). If the solve fails/times out (None),
            # SKIP the ticket entirely (no greedy fallback). Append-only + same gate
            # + same dispatch args (order=S, nearmiss=8, dl=cap); min-wins protects
            # the floor and every preceding draw stays byte-identical.
            if ((len(blocks_data) >= 250 or overload > 1.05)
                    and time.time() < cap):
                try:
                    # short CP-SAT budget, reusing the v20 caller's pattern
                    # (floor a few seconds, bounded by the room left to `cap`).
                    _pt_budget = max(1.0, min(6.0, cap - time.time()))
                    _tgt = _plan_targets(prob_info, bays, PLAN_CAP_FRAC,
                                         _pt_budget)
                    if _tgt is not None and time.time() < cap:
                        _pd = prob_info["blocks"]
                        S_plan = sorted(range(len(_pd)),
                                        key=lambda bi: (_tgt.get(bi, 0),
                                                        int(_pd[bi]["due_date"]),
                                                        bi))
                        # plan_obj1 = the relaxation's OWN tardiness for its
                        # returned target starts (Sum max(0, start+proc-due));
                        # _plan_targets exposes only the times, not its objective.
                        plan_obj1 = sum(
                            max(0, _tgt.get(bi, 0) + int(_pd[bi]["processing_time"])
                                - int(_pd[bi]["due_date"])) for bi in range(len(_pd)))
                        res = dispatch(1.0, 0.5, alpha=1.0, nearmiss=8,
                                       order=S_plan, dl=cap)
                        import os as _os3
                        if _os3.environ.get("OGC_DEBUG"):
                            try:
                                o_fl, a_fl = res
                                champ = (min(cands, key=lambda c: c[0])[1]
                                         if cands else a_fl)
                                corder = sorted(champ.keys(),
                                                key=lambda bi: (champ[bi]["entry_time"],
                                                                bi))
                                rf = {bi: i for i, bi in enumerate(S_plan)}
                                rc = {bi: i for i, bi in enumerate(corder)}
                                md = (sum(abs(rf[bi] - rc.get(bi, rf[bi]))
                                          for bi in rf) / max(1, len(rf)))
                                print("[OGC v29d plan-order] inst_n=%d plan_obj1=%d "
                                      "realized_obj=%.0f mean_rank_delta=%.1f"
                                      % (len(blocks_data), int(plan_obj1), o_fl, md),
                                      flush=True)
                            except Exception:
                                pass
                        cands.append(res)
    ...
    if wid == 3 and raster is not None and forced and len(blocks_data) >= 250:
        ...
        if nm_elig:
            try:
                # v29a: the first-seed slot (proven worthless as a lottery ticket
                # -- 26b + 28c byte-flat twice) becomes an ORDER-FORCED REPLAY
                # SEARCH over the champion's realized admission sequence. Budget =
                # the old slot's 0.22*window deadline. Every replay candidate is
                # pushed through the obj-gated `push` (inside dispatch); the local
                # best raw feeds the UNCHANGED second (ovh) seed + improve below.
                # Dedicated rng (9291) -> no other stream is displaced.
                rr = random.Random(9291)
                slot_dl = t_start + 0.22 * window
                # (i) current inbox champion, else one plain beam dispatch.
                champ_o = None
                champ_a = None
                if inbox is not None:
                    try:
                        while True:
                            o_in, a_in = inbox.get_nowait()
                            if champ_o is None or o_in < champ_o:
                                champ_o = o_in
                                champ_a = {k: dict(v) for k, v in a_in.items()}
                    except Exception:
                        pass
                if champ_a is None:
                    champ_o, champ_a = dispatch(1.0, 0.5, alpha=1.0, beam=True,
                                                dl=slot_dl)
                # (ii) realized admission sequence S (entry order, id tie-break).
                best_o, best_a = champ_o, champ_a
                best_S = sorted(best_a.keys(),
                                key=lambda bi: (best_a[bi]["entry_time"], bi))
                # (iii-v) replay loop: perturb -> order-forced dispatch -> keep-if-
                # better. dispatch pushes each raw; min-wins banks improvements.
                pc = 0
                while time.time() < slot_dl:
                    exit_of = {bi: best_a[bi]["exit_time"] for bi in best_a}
                    S2 = _replay_perturb(best_S, exit_of, blocks_data, rr, pc)
                    pc += 1
                    try:
                        o2, a2 = dispatch(1.0, 0.5, alpha=1.0, nearmiss=8,
                                          order=S2, dl=slot_dl)
                    except Exception:
                        continue
                    if o2 < best_o:
                        best_o, best_a, best_S = o2, a2, S2
                _o, a = best_o, best_a
                # second (ovh) seed + improve: byte-exact v28 (unchanged).
                try:
    ...
    if (wid == 1 and raster is not None and forced and w1 >= 6000
        ...
        try:
            ...
            # v26c (FAILED, reverted): mpc x near_k=32 + nk{48,64} tickets
            # APPENDED here regressed 38 by +1.89M even after a byte-neutral
            # near_k save/restore guard -- the fixed spot's 38 output was
            # byte-identical to 26a's failure. Diagnosis: 38's banked winner
            # is a CROSS-WORKER COUPLING (W1 rotation pushes a near_k build
            # into the island inbox at a specific time; W0-reclaimed's deep
            # polish drains it); perturbing EITHER side -- even append-only
            # time consumption before the jitter tail -- loses it. LAW: the
            # W1 ticket list and W0-reclaimed pacing are both FROZEN for 38.
