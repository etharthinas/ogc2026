...
# myalgorithm_jv13.py -- jv13 = jv9 + GIANT SOLO (island broadcast off on giants).
# Tests whether independent basin exploration finds giant solutions at/below
# the converged best (jv12 proved the portfolio collapses to one basin/run).
# OGC_GIANT_SOLO=0 -> jv9. See heuristic_jv13.md.
# ---------------------------------------------------------------------------
...
def _algorithm_portfolio(prob_info, timelimit, t_start):
    ...
    if len(prob_info["blocks"]) >= 250 and _is_forced(prob_info, _pre_bays):
        ...
        # jv13 GIANT SOLO: suppress the island broadcast on giants so the 8
        # workers explore 8 INDEPENDENT basins to completion instead of all
        # adopting the first-good leader and collapsing to ONE basin (jv12
        # proved best5 identical within a run). Hypothesis: convergence makes
        # the portfolio UNDER-explore -- an independent worker may find a basin
        # BELOW the converged best (not just reduce the 27 +/-425k variance).
        # min-wins keeps the best of 8. OGC_GIANT_SOLO=0 disables (== jv9).
        _giant_solo = __import__("os").environ.get("OGC_GIANT_SOLO", "1") != "0"
    else:
        _giant_solo = False
    ...
    while time.time() < search_deadline:
        try:
            ...
            if item[0] < gbest * 0.998:
                gbest = item[0]
                if not _giant_solo:        # jv13: giants explore independently
                    for ib in inboxes:
                        try:
                            ib.put(item)
                        except Exception:
